 ...
 * create-zudo-doc.
 *
 * This is a HAND-MAINTAINED, path-idiom guard, not an exhaustive scan: it
 * only catches the specific monorepo-only strings listed below. It must be
 * grown by hand whenever a NON-PATH monorepo idiom (the maintainer's
 * personal Claude Code skills/tooling — e.g. `/verify-ui`, `/l-*`,
 * `/test-wisdom` — or repo-wide conventions like `lefthook`, `run-p`,
 * `save-file.js {logdir}`) gets re-synced into a LINT_SKILLS template from
 * the monorepo's own `.claude/skills/` source; only `/css-wisdom` is
 * currently caught below. This fast denylist lint and the
 * `claude-skills-scaffold-refs` integration guard (real-scaffold,
 * path-based — see that file) are COMPLEMENTARY, not redundant: this one
 * catches known-bad strings cheaply without a scaffold build; that one
 * catches unknown dead path references by actually generating a project.
 */
...
describe("claudeSkills byte-mirror parity mechanism (PARITY_SKILLS; empty as of #2948)", () => {
