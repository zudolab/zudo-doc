...
describe("scaffold — .gitignore base blocks", () => {
  it("always ignores node_modules, dist, .zfb, .zfb-build/, .env*, and .wrangler/", async () => {
    await scaffold(baseChoices);
    const gitignore = await fs.readFile(projectPath("test-doc", ".gitignore"), "utf-8");
    for (const line of [
      "node_modules",
...
describe("scaffold — settings-drift guard: generator-known fields must cover every ZudoDocConfig field", () => {
  ...
  it("every ZudoDocConfig field is generator-known (wired OR explicitly allowlisted)", async () => {
    ...
    const ALLOWLIST: Record<string, string> = {
      ...
      bundleZdtp:
        "hand-authored post-scaffold — only a host mounting its own panel through design-token-panel-bootstrap sets it; the scaffold never wires a host panel (#4261)",
      ...
      favicon: "no CLI/prompt surface — the omitted default (the scaffold's shipped four-file public/ convention) is correct for all scaffolds; hand-edit post-scaffold",
      ...
      docMetainfoFields: "no CLI/prompt surface yet — hand-edit post-scaffold; advanced metadata display control",
      ...
      docHistoryUi: "no CLI/prompt surface yet — hand-edit post-scaffold to keep history dates while disabling the history UI",
