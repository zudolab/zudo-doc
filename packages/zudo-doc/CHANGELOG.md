...
- `--zdc-sidebar-font` / `--zdc-toc-font` are authored to target both a desktop and a mobile selector, but only the desktop reach is delivered today: the mobile drawer (`aside[data-zd-mobile-sidebar]`) and mobile TOC (`div[data-zd-mobile-toc]`) are zfb client islands whose `data-*` attributes are stripped from SSR output, so those mobile surfaces always render on the `--zdc-chrome-font` seam base regardless of a granular override. Tracked as #2898 (epic #2886)
