...
- `--zdc-sidebar-font` / `--zdc-toc-font` are authored to target both a desktop and a mobile selector, but only the desktop reach is delivered today: the mobile drawer (`aside[data-zd-mobile-sidebar]`) and mobile TOC (`div[data-zd-mobile-toc]`) are zfb client islands whose `data-*` attributes are stripped from SSR output, so neither selector ever matches. Each mobile surface instead inherits from its real DOM ancestor — the drawer follows `--zdc-header-font` (it renders inside `header[data-header]`), the mobile TOC follows `--zdc-doc-prose-font` (it renders inside `.zd-content`) — not a uniform fallback to `--zdc-chrome-font`. Tracked as #2898 (epic #2886)
