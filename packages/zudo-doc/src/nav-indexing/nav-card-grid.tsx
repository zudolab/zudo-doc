...
//   - The arrow SVG uses `text-muted` colouring (accent on hover/focus) and
//     appears to the left of the label (gap-hsp-sm, items-center) rather than
//     the CategoryNav flex-start layout.
//   - Description has a `group-hover:underline` effect on the span.
//
...
function ArrowIcon(): JSX.Element {
  return (
    <svg
      ...
      aria-hidden="true"
      class="w-[16px] shrink-0 text-muted group-hover:text-accent group-focus-visible:text-accent"
    >
 ...
 * hover/focus) and sits inline with the label via `items-center`. Description
 * lines have a hover-underline effect.
 *
      ...
      {items.map((child, i) => (
        ...
        >
          <span class="flex items-center gap-hsp-sm">
            <ArrowIcon />
            <span class="font-medium text-fg group-hover:text-accent group-hover:underline group-focus-visible:text-accent group-focus-visible:underline">
              {child.label}
            </span>
          </span>
          {child.description && (
            <span class="mt-vsp-2xs block text-small text-muted group-hover:text-accent group-hover:underline group-focus-visible:text-accent">
              {child.description}
            </span>
          )}
        </a>
