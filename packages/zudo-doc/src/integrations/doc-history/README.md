...
- **Dev proxy** — forwards `/doc-history/*` HTTP requests to the
  standalone `@takazudo/zudo-doc-history-server` running on port `4322`.
- **Post-build hook** — spawns the `doc-history-generate` CLI from
  `@takazudo/zudo-doc-history-server` to emit per-page JSON files under
...
import { docHistoryPlugin } from "@takazudo/zudo-doc/integrations/doc-history";
...
path of `doc-history-generate` from `@takazudo/zudo-doc-history-server`'s
...
`doc-history-generate` CLI shipped by `@takazudo/zudo-doc-history-server`
