...
// ───────────────────────────────────────────────────────────────────────────
// IMPORT-GRAPH GUARD for standalone.ts (issue #3048).
//
...
// So `standalone.ts`'s module graph must never reach a `.tsx`-derived module
// or `preact` itself (both would either fail to import outside a JSX
// pipeline or drag the preact runtime into a plain CLI script). This mirrors
// the node-free eval-graph guard pattern in `src/__tests__/preset.test.ts`
// (bundle exactly the reachable graph, assert on what's reachable) but
// checks reachable *input files* via esbuild's metafile instead of
// `node:*` specifiers.
// ───────────────────────────────────────────────────────────────────────────
...
describe("standalone.ts's module graph is .tsx-free and preact-free", () => {
  it("bundles standalone.ts and reaches no .tsx module and no preact", async () => {
    const esbuild = loadEsbuild();
    const result = await esbuild.build({
      entryPoints: [standaloneSrc],
      bundle: true,
      write: false,
      platform: "neutral",
      format: "esm",
    ...
    expect(tsxOffenders, `standalone.ts reached .tsx module(s): ${tsxOffenders.join(", ")}`).toEqual([]);
    expect(preactOffenders, `standalone.ts reached preact module(s): ${preactOffenders.join(", ")}`).toEqual([]);
  });
