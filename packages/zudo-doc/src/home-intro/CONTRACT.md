...
| Directives, transclusion, filesystem includes | Rejected by the raw parser even when configured for document pages. Bare inline colon tokens attached to a word or number (`09:30`, `3:1`, `word:word`) are ordinary prose and remain literal text. Standalone directives and directives with labels or attributes (including empty `[]` or `{}`) remain rejected. Custom directive component mappings and source-relative includes require a document source, which this field deliberately has none of. |
