...
| `./chrome-bindings` | `defineChromeBindings(input)` — the compile-time-checked constructor for a host's `ChromeHostBindings` object, plus the `ChromeBindingsInput` type. Build `chromeBindings` with this instead of a raw object literal or an `as`/`as unknown as` cast on `ChromeHostBindings`: `ChromeBindingsInput` types each slot to its exact call-site contract, so a value that doesn't accept the props the chrome actually passes is a compile error (drift detection, #2674); the widening to the full `ChromeHostBindings` shape happens exactly once, internally |
