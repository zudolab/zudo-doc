...
| `favicon?` | `string \| FaviconConfig \| false` | `<link rel="icon">` set. Omitted emits the four-file convention (`/favicon.svg`, `/favicon.ico`, `/favicon-32x32.png`, `/favicon-16x16.png`); `"auto"` emits one inline SVG data-URL icon seeded by `siteName`; any other string emits a single link with `type` inferred from the extension; a `FaviconConfig` (`{ svg?, png32?, png16?, ico? }`) emits only the slots it supplies, always in `svg → ico → png32 → png16` order; `false` emits none. The empty string `""` — top-level or in any `FaviconConfig` slot — throws a `TypeError` at config resolution (an empty `href` resolves to the current document per the HTML spec); an empty object `{}` is unaffected and emits nothing. |
...
| `--color-page-loading-spinner` | Page-loading spinner ring (pinned light — the scrim is theme-independent) |
