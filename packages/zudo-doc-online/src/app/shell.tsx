...
import { formatRoute, routeProjectSlug, type Route } from "./router.js";
...
export interface ShellProps {
   ...
   * spec, a mount without a live server, or a route with no project context
   * (`projects`/`new-project`) gets.
   */
...
export function Shell({ route, store = null, events, storage, children }: ShellProps) {
  const { pageId: editorPageId, projectTitle } = useEditorEntryPageId(route, {
    store,
    events,
    storage,
  });
  ...
  return (
    ...
    <div className="flex h-dvh flex-col bg-bg text-fg">
      <header className="flex items-center gap-hsp-lg border-b border-border px-hsp-xl py-vsp-xs">
        <a
          href={formatRoute({ name: "projects" })}
        ...
        {projectSlug === null ? null : (
          <span className="text-small text-muted">{projectTitle ?? projectSlug}</span>
        )}
        {projectSlug === null ? null : (
          <nav className="flex gap-hsp-md" aria-label="Primary">
            ...
            {editorPageId === null ? (
              ...
              <a
                href={formatRoute({ name: "editor", projectSlug, pageId: editorPageId })}
