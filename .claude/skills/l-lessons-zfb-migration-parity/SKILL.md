...
## 2026-07-10 — Minimal Scaffold (epic #2651): spike-first, wave-split, and the confirm-gate pattern paying for itself twice

### What we set out to do

Shrink `create-zudo-doc`'s scaffold from a 64-file "reference implementation with wiring shells" to a locked ~12-file minimal manifest (one config file, markdown, a handful of unavoidable root files — everything else shipped from `@takazudo/zudo-doc` in `node_modules`), migrate the showcase itself to the same single-config shape, and ship it across 9 sub-issues / 9 waves with two confirm gates (#2659 mechanical floor, #2667 — this one — end-to-end).

### Approach

Wave 1 was a pure-observation spike (#2652, opus model — "verification errors cascade into every downstream spec") against the exact pinned zfb version, answering 6 floor-mechanics questions with runnable evidence before any downstream spec was written. Wave 2 (#2653, opus) converted the spike's findings into a **locked, filename-exact spec** appended to all 14 downstream sub-issue bodies, so implementation waves had no ambiguity to improvise around. Waves 3–8 then executed package-foundations → package-API → integration-confirm → generator/showcase → tests/infra → docs/skills, in dependency order, each posting a structured completion comment with deviations flagged loudly. Wave 9 (this sub-issue) re-ran everything against the fully merged state: all 9 generator CLI patterns rebuilt from scratch against a packed local `@takazudo/zudo-doc` tarball, the full 21-step `pnpm b4push`, and a manual requirements walk against the epic's own "what goes away" table.

### What worked

- **The spike caught real, load-bearing bugs before they could propagate into 14 sub-issue specs.** Three concrete catches: (1) a stale code comment at `packages/zudo-doc/src/plugins/routes.ts:~127` was written against zfb `next.62` and claimed `/` injection was rejected in dev — spike Q1 proved this was stale for `next.78` (build renders `/`; dev silently drops it, not a hard error); (2) spike Q1 also surfaced the **inverse** latent bug that ended up mattering most: injected **dynamic** doc routes (`/docs/[[...slug]]`) 404 in `zfb dev` even though injected *static* routes now render fine — this single finding is the entire reason the locked manifest keeps a self-contained `pages/docs/[[...slug]].tsx` stub instead of reaching true zero-pages-for-docs; (3) spike Q5 found the tsconfig `files`-vs-`include` override trap (a base tsconfig's `include` is silently discarded, not merged, by a project's own `include` — the ambient shims MUST ship via base `files`, or a planted typecheck error passes `zfb check` clean). All three are exactly the kind of thing that's cheap to fix in a spike and expensive to unwind after 6 downstream sub-issues have already coded against the wrong assumption. **Current status:** zfb 2.13.1 later resolved dynamic injected-route rendering in dev; the retained stubs are now explicit host-owned seams, not 404 workarounds.
- **The wave-split forced by plan review held.** The original plan bundled package foundations and package API into one wave; Opus review flagged a hidden intra-wave dependency (the API sub imports the defaults modules from Wave 3, and the designTokenPanel sub imports the theme-CSS token manifest from the same wave) and split it into Wave 3 (#2654 defaults / #2655 theme CSS / #2656 tsconfig floor, parallel) → Wave 4 (#2657 `zudoDoc()` API / #2658 designTokenPanel island, parallel, each consuming a specific Wave-3 output). Concretely: `s6-dtp-island` (#2658) could not have started before `s5-theme-css` (#2655) landed the token manifest it derives Font/Spacing/Size tabs from. Nobody hit a "my import doesn't exist yet" wall during implementation — the split was correct.
- **The chrome-seam gate-2 miss + confirm-gate catch is the cleanest demonstration of why two confirm gates were budgeted.** #2658 shipped `DesignTokenPanelBootstrap` wired explicitly into the package's own `routes/_chrome.tsx`, verified against every *injected* route — and missed that the locked-manifest's self-contained doc stub calls `createChrome(routeCtx)` with **no** `hostBindings`, so the island silently never mounted there (a documented "safe no-op", not a crash — exactly the "dead islands are silent" failure mode this project's lessons already warn about, just shaped as "missing" instead of "orphaned"). The Wave-5 confirm gate (#2659) caught it with an apples-to-apples island-set diff between the stub and the injected-route baseline, filed a precise blocking comment with root cause + suggested fix shape (not a directive) on #2658, which shipped a package-seam fix (default the hostBed slot in `chrome/derive.tsx` itself, so every `createChrome` caller inherits it) within the same wave. #2659 re-ran and flipped from FAIL to PASS. This is the gate working exactly as designed — caught before Wave 6 generator/showcase shipped `designTokenPanel: true` through a broken path.
- **The zdtp hard-dependency tradeoff was accepted with eyes open, not silently absorbed.** The gate-2 fix's unconditional static import of `DesignTokenPanelBootstrap` at the chrome seam means `@takazudo/zdtp` becomes a build-time-resolvable dependency of *every* generated project, even `designTokenPanel: false` ones (discovered by #2660 when a barebone scaffold's `zfb build` failed with an unresolved import). The generator-side workaround (add zdtp as an unconditional dep in `generatePackageJson()`) unblocked every scaffold immediately; the underlying package-side coupling was filed as #2668 rather than papered over. Same pattern as the pre-existing `diff`/DocHistory precedent this repo already had — recognized and reused, not rediscovered from scratch.

### Watch for next time

- **Cross-wave "parity/moved-data" tests need an explicit expiry, not just a creation date.** Wave 3 (#2654) wrote 6 test files that regex-extracted data out of the *old* template files as a parity guard for the initial data-module port. Wave 6 (#2660) legitimately deleted those exact template files 3 waves later as part of the manifest shrink — and nothing flagged that the parity tests' target had gone. All 6 files failed at Wave 9 (this gate): 4 with a clean assertion failure, 2 with the whole suite crashing at module-load time (the file-read happened outside any `it()`, at `describe()`-body top level). **The reason nobody caught this for 3 waves: `pnpm -r --filter './packages/*' run test` stops at the FIRST failing package by default** (`ERR_PNPM_RECURSIVE_RUN_FIRST_FAIL`) — every wave that ran `pnpm test`/`pnpm b4push` on a red `create-zudo-doc` package (which was red for unrelated, already-fixed reasons at various points) never even reached `packages/zudo-doc`'s own suite in the same run, so the drift accumulated invisibly. A parity test whose comparison target is scheduled for deletion should either (a) live in the SAME wave/PR as the deletion with instructions to delete both together, or (b) assert against a stable in-repo invariant instead of a sibling file path that a later wave owns.
- **A skill's own worked-example CLI commands need to be run, not just written.** The `l-generator-cli-tester` skill's `light-dark` and `all-features` patterns both passed `--light-scheme "Default Light" --dark-scheme "Default Dark"` with no explicit `--default-mode`, which resolves to the exact `ZudoDocConfig` package default for `colorMode` — so the (correctly implemented, correctly unit-tested) diff-from-defaults generator omits the field entirely, silently degenerating the pattern's own "verify colorMode is emitted" assertion into a false negative. The unit test suite (`zfb-config-gen.test.ts`) got this right from day one (it deliberately tests both the exact-default case, expecting omission, and a non-default case, expecting emission) — the skill's hand-written example just didn't follow its own authoritative test file's pattern. Skills with runnable example commands are docs with an untested code path; either CI-exercise them or budget one live run per rewrite before merging.
- **A cross-wave doc/type reconciliation commit fixed the prose but missed the test.** `designTokenPanelConfigModule` was documented by #2664 as settings-spread-only (not on `ZudoDocConfig`), while #2665 — running in parallel — surfaced it as a first-class `ZudoDocConfig` field. A later reconciliation commit (`d5adb57`, landed at merge time) updated the EN/JA doc prose to match the merged reality, but the generator's own `scaffold.test.ts` "every `ZudoDocConfig` field is generator-known" drift guard — which predates #2665's change — was never told about the new field, and failed red at this gate. Prose reconciliation and mechanical drift-guard reconciliation are two different fixes; a parallel-wave field-surfacing change needs a checklist that includes "does this field affect the census used by the OTHER package's tests," not just "did I update the docs that mention it."

### Would-skip-if-redoing

- Would skip letting a "moved data parity" test's `readFileSync(templatePath)` call sit at `describe()`-body top level (executes at collection time, before any `it()` can catch it) — even a soon-to-be-obsolete parity test should fail inside an `it()` so a suite-wide crash doesn't mask everything else in the file.
- Would skip trusting `pnpm -r`'s default fail-fast semantics as "the whole workspace passed" — a wave that needs "did package X regress" signal should target that package directly (`pnpm --filter <pkg> test`) rather than infer it from a green `pnpm -r` that may never have reached it.
- Would skip hand-picking CLI flag values for a skill's worked example without cross-checking them against the diff-from-defaults semantics the generator implements — a documented CLI command whose output happens to equal the default is not exercising anything.

### References

- Epic: #2651 (sub-issues #2652–#2667)
- Confirm gates: #2659 (Wave-5 mechanical floor), #2667 (Wave-9 end-to-end, this entry)
- Chrome-seam fix: `packages/zudo-doc/src/chrome/derive.tsx` (`deriveBodyEndIslands`)
- Fixed-at-gate test debt: `packages/zudo-doc/src/__tests__/{i18n,z-index,docs-schema,color-schemes,directive-vocabulary,frontmatter-preview}-defaults.test.ts`, `packages/create-zudo-doc/src/__tests__/scaffold.test.ts` (ALLOWLIST)
- Follow-ups filed: #2668 (zdtp hard dep), #2669 (tauri find-in-page unwired), #2670 (upstream zfb dev-render pointers), #2671 (md-plugins retirement), #2672 (pre-existing pages/ typecheck bug)
