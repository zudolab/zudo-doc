...
- **ViewTransitions Strategy A2 was the right call.** A2 = zfb runtime emits a typed no-op + host adds the CSS at-rule. The alternative A1 (zfb emits the `<style>@view-transition{}</style>` itself) was materially equivalent and would have been adopted if the upstream maintainer pushed back; they didn't. Strategy B (full SPA soft-swap router, Astro-style) was correctly classified as multi-month upstream effort and rejected. Strategy C (CSS-only fade, no VT) abandoned the morphing-sidebar requirement and was rejected. Browser verification on PR #1504 preview confirmed all 5 contract assertions PASS (native `startViewTransition`, CSS rule present, meta tag absent, inline IIFE absent, `pageswap.event.viewTransition` truthy).
...
- **For ViewTransitions specifically: the Playwright `pageswap` event check is the highest-signal contract assertion.** `event.viewTransition` truthy iff Chromium captured the navigation. The 4 console assertions (native API, CSS rule, no meta, no IIFE) are all SSR-shape checks that pass even on broken impls; only `pageswap.viewTransition` proves the live navigation was captured. Make this the canonical first check for any future cross-document VT debugging.
