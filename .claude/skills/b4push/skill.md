...
name: b4push
description: >-
  Run comprehensive pre-push validation covering type checking, build, and
  E2E/smoke tests. Use when: (1) Completing a PR or feature implementation,
  (2) Before pushing significant changes, (3) After large refactors,
...
1. **Type checking** — `pnpm check` (astro check with strict TypeScript)
2. **Build** — `pnpm build` (static HTML export to dist/)
3. **E2E & smoke tests** — `pnpm test:e2e` (Playwright tests including page smoke tests)
