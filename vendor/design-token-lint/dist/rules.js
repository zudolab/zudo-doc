 ...
 */
import { DEFAULT_CONFIG, compileConfig } from './config.js';
// Default compiled config (used when no external config is loaded)
let activeConfig = compileConfig(DEFAULT_CONFIG);
/**
 ...
 */
export function checkClassWithConfig(className, config) {
...
function matchRule(originalClassName, withoutNeg, rule) {
