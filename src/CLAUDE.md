...
- Current LOCAL client islands (`src/components/`): `sidebar-toggle.tsx`, `client-router-bootstrap.tsx`, `desktop-sidebar-toggle.tsx`. The design-token panel bootstrap and everything else that used to be a local island — `sidebar-tree.tsx`, `doc-history.tsx`, `image-enlarge.tsx`, `ai-chat-modal.tsx`, ThemeToggle, Toc, MobileToc — are now PACKAGE-OWNED (`@takazudo/zudo-doc/{design-token-panel-bootstrap,sidebar-tree-island,doc-history,image-enlarge,ai-chat-modal,theme-toggle,toc}`); their npm-dist `"use client"` modules are scanned by zfb >= 0.1.0-next.39 (zfb#999/#1001), so no local scanner-visible shims exist (re-adding one creates an island marker-name collision). Every ejectable component is listed in `packages/zudo-doc/src/eject/index.ts`'s `EJECTABLE` map.
