...
- Island wiring locations (package-owned since the minimal-scaffold cutover, epic zudolab/zudo-doc#2651): the doc-route stubs (`pages/docs/[[...slug]].tsx` and its `[locale]`/`v/**` variants) call `createChrome(routeCtx, chromeBindings)` (`@takazudo/zudo-doc/chrome`), which builds the header/sidebar/toc/footer chrome and mounts `BodyEndIslands` (ClientRouterBootstrap, DesignTokenPanelBootstrap, AiChatModal, ImageEnlarge, MermaidEnlarge) internally — there is no more project-side `_header-with-defaults.tsx` / `_sidebar-with-defaults.tsx` / `_doc-page-shell.tsx` wrapper (those moved into the package as `header-with-defaults/`, `sidebar-with-defaults/`, `doc-page-shell/`). `pages/lib/_body-end-islands.tsx` (this repo's own showcase-only real body-end-islands implementation, unlike a fresh scaffold which has none) is threaded in via `src/chrome-bindings.tsx`'s `BodyEndIslands` slot, wired through `zfb.config.ts`'s `chromeBindingsModule` setting. `DesignTokenPanelBootstrap` is auto-defaulted at the package's `chrome/derive.tsx` seam for every `createChrome` consumer (no host wiring needed even in a bare scaffold); `DocHistory` is NOT auto-defaulted (its package default is a deliberate no-op stub) — a project that wants real doc-history data must thread it through `chromeBindingsModule`, same as this showcase does.
- Current LOCAL client islands (`src/components/`): `sidebar-toggle.tsx`, `client-router-bootstrap.tsx`, `design-token-panel-bootstrap.tsx`, `desktop-sidebar-toggle.tsx`; the zdtp panel self-mounts via `configurePanel()` and needs no Island wrapper. Everything else that used to be a local island — `sidebar-tree.tsx`, `doc-history.tsx`, `image-enlarge.tsx`, `ai-chat-modal.tsx`, ThemeToggle, Toc, MobileToc — is now PACKAGE-OWNED (`@takazudo/zudo-doc/{sidebar-tree-island,doc-history,image-enlarge,ai-chat-modal,theme-toggle,toc}`); their npm-dist `"use client"` modules are scanned by zfb >= 0.1.0-next.39 (zfb#999/#1001), so no local scanner-visible shims exist (re-adding one creates an island marker-name collision). Every one of these is also individually ejectable via `zudo-doc eject <name>` if a project needs to fully own the source (see `packages/zudo-doc/src/eject/index.ts`'s `EJECTABLE` map).
- Content typography components: server-rendered Preact functions that override HTML elements emitted by MDX (headings h2-h4, paragraph, link, strong, blockquote, lists ul/ol, table). Since the package-first migration (epic #2321) this whole system — the component implementations AND the `<Content components={...} />` mapping (`component-map.ts`) — is package-owned (`packages/zudo-doc/src/content/`, wired by `@takazudo/zudo-doc/mdx-components`); there is no more host `pages/_mdx-components.ts`. `src/components/content/code-group.tsx` and `content-admonition.tsx` are thin re-export shims (`export { CodeGroup } from "@takazudo/zudo-doc/code-group"`, etc.) kept so existing `@/components/content/*` call sites stay unchanged — they are NOT part of the override registration anymore.
