...
describe("showcase package changelog hierarchy", () => {
  it("keeps the bilingual package release trees aligned", () => {
    ...
    expect(releases).toHaveLength(109);
    expect(releases.filter((path) => path.startsWith("zudo-doc/"))).toHaveLength(99);
    expect(releases.filter((path) => path.startsWith("create-zudo-doc/"))).toHaveLength(5);
  ...
  it("uses a root landing and newest-first package lanes", () => {
    ...
    expect(zudo?.children[0]?.slug).toBe("changelog/zudo-doc/5.13.1");
