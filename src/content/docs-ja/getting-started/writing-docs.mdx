...
デフォルトのスキャフォールドは1つのロケールだけで、`src/content/docs-ja/` ディレクトリもロケールルートもありません。スキャフォールディング時に `i18n` 機能を選ぶと、セカンダリ言語、そのミラーリングされたコンテンツディレクトリ、`pages/[locale]/docs/[[...slug]].tsx` が追加されます。後から `zudoDoc({ ... })` の `locales` フィールドでロケールを追加することもできます。
