...
この 2 つの出発点を超えてパレットをカスタマイズするには、スキャフォールド後に `src/config/color-schemes.ts` でランプとセマンティックマップを編集します — [Color](../reference/color.mdx#カスタムカラースキームの追加) を参照し、[Design Token Panel](../reference/design-token-panel.mdx) の Palette タブで変更をライブプレビューできます（[カラースキームプレビュー](../guides/color-scheme-preview.mdx)を参照）。
