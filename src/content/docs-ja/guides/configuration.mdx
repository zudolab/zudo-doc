...
tauri フィーチャーのスキャフォールドでは `create-zudo-doc` が自動的に有効化します。`window.__TAURI_INTERNALS__` で自己ゲートするため、`true` の場合でも Tauri シェル外では安全に何もしません（no-op）。`BodyEndIslands` クロームバインディングスロットを上書きするホストは（[ホストクロームバインディング](../reference/host-chrome-bindings.mdx)を参照）、アイランド自体をマウントする必要があります — パッケージのデフォルトはスロットが未設定の場合にのみ適用されます。
