...
`/` や `/ja` のようなロケール接頭辞付きホームページで、パッケージ所有のカテゴリグリッドから外し、その直下のコンパクトなセカンダリ行へ移すトップレベルカテゴリのスラッグです。指定したカテゴリはそれぞれ、ラベル付きの小さなシェブロンリンクとして表示されます。行内の並び順はグリッドのカテゴリ順ではなく、配列の順序に従います。ホームページから辿れるようにはしておきたいものの、主要な読み進めの導線とは競合させたくないシステム系のカテゴリに使います。
...
- どのトップレベルカテゴリにも一致しないスラッグは無視されます。
- [`siteTreeNavIgnore`](#ホームページ-sitetreenavignore) にも含まれるスラッグは完全に非表示になります。`siteTreeNavIgnore` が優先されます。
- 自身のページを持たないカテゴリ（[`category_no_page: true`](./frontmatter.mdx#フィールド詳細-category_no_page)）はリンク先がないため、グリッドに残ります。このとき開発時の警告が出力されます。
- リストが空の場合、セカンダリ行は表示されません。

この設定はホームページにのみ影響します。`<SiteTreeNav />` / `<SiteTreeNavDemo />` MDX タグはこの設定を無視し、ヘッダーナビ、サイドバー、検索、サイトマップも変わりません。
...
検査対象は`/`で始まるサイト内の絶対パスです。クエリとフラグメントを除去し、URLエンコードを復号して、ビルド出力内のファイルを確認します。`base: "/manual/"`なら`/manual/img/logo.svg`は`dist/img/logo.svg`に対応します。baseの外を指すパス、存在しないファイル、出力ディレクトリの外へ抜けるパスは問題として報告されます。

外部URL、`//`で始まるURL、`data:`などのスキーム付きURL、相対パスは対象外です。コメント、スクリプト内の文字列、エスケープされたコード例も画像として扱いません。`srcset`は検査しません。
...
| プロパティ | 型 | 説明 |
|----------|------|-------------|
| `defaultMode` | `"light" \| "dark"` | System に従わず、保存済みの選択がない場合に使うライト/ダークのフォールバック |
| `lightScheme` | `string` | ライトモードで使うスキーム |
| `darkScheme` | `string` | ダークモードで使うスキーム |
| `respectPrefersColorScheme` | `boolean` | 保存済み設定がない初回訪問で System を選ぶ |

デフォルトの `respectPrefersColorScheme: true` では、保存済みの選択がない初回訪問時に **System** が選ばれ、デバイスのライト/ダーク設定に従います。コンパクトなヘッダーの外観メニューでは **Light**、**Dark**、**System** を選べます。アイコンは次に選ばれる設定ではなく、現在選択されている設定を表します（Light は太陽、Dark は月、System はモニター）。選択は `localStorage` の `zudo-doc-theme` に保存され、再読み込み後も維持されます。

`defaultMode` は引き続きライト/ダークの2値フォールバックです。`respectPrefersColorScheme: false` にすると、保存済みの選択がない初回訪問時は `defaultMode` が使われます。利用者はメニューから System を明示的に選ぶこともでき、その場合は引き続きデバイス設定に従います。`colorMode: false` は単一の固定スキームを保ち、外観メニューは表示しません。
...
テーマパックは、上記のカラースキームシステムの**上に重なる**インストール可能なデザインバンドルです — すべてのパックがライト/ダーク両方の値を定義するため、外観メニューはどのパックでもそのまま機能します。スイッチャー UI、theme CLI、パックの作り方は[テーマパックリファレンス](../reference/theme-packs.mdx)を参照してください。
...
`math: true` にはオプショナルピア依存の `katex` のインストールが必要です（`pnpm add katex`）— `create-zudo-doc` は自動で追加せず、新規 scaffold の `package.json` には含まれません。`katex` は `@takazudo/zudo-doc` のオプショナルピアです。インストールしなくてもビルドは通りますが、実行時に `<MathBlock>` のレンダリングが例外を投げます。コンポーネントについては[数式](../components/math-equations.mdx)を参照してください。`docHistory` の Compare ビューも `diff` について同様の契約を持ちます — 下記の[docHistory](#サイト設定-dochistory)を参照してください。
...
検索マッチングのために各ページの本文をインデックスする最大文字数。

型: `number` · デフォルト: `3000`

これが制御するのは、検索クエリがページのどこまで深くマッチできるかであり、結果にどれだけの本文が表示されるかではありません — 結果には常にマッチ箇所を中心としたコンパクトなスニペットが表示されます。詳しくは[サイト検索](./search.mdx)を参照してください。上限を上げると、その分だけ`search-index.json`も大きくなります。たとえば、以前のデフォルトである300文字から3,000に上げると、このショーケースではインデックスサイズがおよそ3.4倍（約410 KBから約1.4 MB）になりました。不正な値（整数でない、または`0`以下）は設定時に例外を投げます。
...
メタデータブロックに表示する項目を選びます。`docMetainfo: true`と、データを提供する`docHistoryMeta`バインディングが必要です。
...
`zfb.config.ts`を手動で編集して設定します。新しいCLIフラグはありません。表示項目を減らしても履歴データの生成は停止しません。
...
| プロパティ | 型 | 説明 |
|----------|------|-------------|
| `docHistory` | `boolean`（任意） | History ボタンを表示（`docHistory: true`と`docHistoryUi: true`が必要） |
...
| `type` | 追加フィールド | 説明 |
|--------|--------------|-------------|
| `"component"` | `component` — `"theme-toggle"`（Light / Dark / System の外観メニュー）, `"language-switcher"`, `"version-switcher"`, `"github-link"`, `"search"` | 組み込みヘッダーコンポーネント |
