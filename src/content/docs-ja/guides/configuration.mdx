...
<Note title="tocToggle と hide_toc の違い">
  ...
  この2つは解決する問題が別で、併用もできます。[`hide_toc`](./frontmatter.mdx#フィールド詳細-hide_toc) フロントマターは**書き手**の判断をビルド時に焼き込むものです。そのページは目次を持たない状態で配信され、読者が呼び戻すことはできません。一方 `tocToggle` は**読者**のための操作です。どのページも目次を持ったまま、いま表示するかどうかを読者が決めます。
