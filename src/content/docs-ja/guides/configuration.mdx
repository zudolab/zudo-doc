...
有効にすると、開発サーバーがトレイリングスラッシュなしのURLをトレイリングスラッシュ付きのURLへ**301リダイレクト**します（例：`/docs/guides` → `/docs/guides/`）。これにより、ローカルでも本番と同じナビゲーション挙動になります。静的アセット（`.js`、`.css`、`.png`などの拡張子を持つファイル）やAstroレガシーパス（`/_astro/`、`/_image` — 既存リンクやキャッシュとの後方互換性のために保持されています。zfbはこれらのプレフィックスを出力しません）はリダイレクトの対象外です。
...
カラースキームは`src/config/color-schemes.ts`で定義されます。各スキームは21のカラープロパティを提供します：

- `background`、`foreground`、`cursor`、`selectionBg`、`selectionFg`
- `palette` — 16色スロット（`palette[0]`から`palette[15]`）
- `semantic`（オプション）— `surface`、`muted`、`accent`、`accentHover`などのオーバーライド
