...
これが制御するのは、検索クエリがページのどこまで深くマッチできるかであり、結果にどれだけの本文が表示されるかではありません — 結果には常にマッチ箇所を中心としたコンパクトなスニペットが表示されます。詳しくは[サイト検索](./search.mdx)を参照してください。上限を上げると、その分だけ`search-index.json`も大きくなります。たとえば、以前のデフォルトである300文字から3,000に上げると、このショーケースではインデックスサイズがおよそ2.5倍になりました。不正な値（整数でない、または`0`以下）は設定時に例外を投げます。
