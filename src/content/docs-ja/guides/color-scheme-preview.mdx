...
カラースキームプレビューは、50種類以上のプリセットカラースキームをブラウズし、リアルタイムでサイトに適用できる機能です。この機能は[Design Token Panel](../reference/design-token-panel.mdx)の**Color**タブにあり、タブヘッダーの "Scheme..." ドロップダウンから呼び出します。
...
各スキームは16色のパレット、背景・前景色、選択色、任意のセマンティックカラー上書き、および任意の `shikiTheme`（レンダリングには影響しない名残りのフィールド。コードハイライトは syntect が担当します。[Syntax Highlighting](../markdown-features/syntax-highlighting.mdx) を参照）を指定します。
