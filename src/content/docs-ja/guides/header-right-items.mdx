...
パッケージの `applyColorScheme`(`theme-toggle/color-scheme-sync.ts` 内)は、カラースキームの変更時に Design Token Panel の状態を意図的に **クリアしません** — ストレージのライフサイクルは `@takazudo/zdtp` が管理します。Palette・Spacing・Font・Size の編集はライト/ダークの切り替えをまたいでそのまま保持され、Color タブの ref/literal のデフォルトは切り替え先のモードに合わせて設定されます。保存された Color のオーバーライドはスキームとモードごとに管理されます。ライトで編集したマッピングがダークのデフォルトや保存済みマッピングを上書きすることはなく、各モードに戻るとそのモードで保存した設定が復元されます。パッケージ標準のビルダーは、`panelSettings.colorMode` に `Default Light` と `Default Dark` をそれぞれ別の識別名として指定しています。詳しいセマンティクスは[モードスコープの編集モデル](../reference/design-token-panel.mdx#タブ構成-color-モードスコープの編集モデル)を参照してください。
