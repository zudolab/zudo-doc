...
`design-token-panel` トリガーボタンは `window` に `toggle-design-token-panel` カスタムイベントをディスパッチします。zdtpパネル（`@takazudo/zudo-design-token-panel`）はこのイベントをネイティブに受け取るため、追加の配線は不要です。
