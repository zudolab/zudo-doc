...
`assetViewerExclude` で除外したファイルは閲覧ページと一覧のどちらにも現れませんが、元の公開 URL からは引き続き取得できます。メタデータサイドカーも一覧には含まれません。一覧ページは個別の閲覧ページと同様、デフォルトロケールにだけ生成されます。
...
`versioned: false` を指定すると、アーカイブ版のプレフィックスを加えず、単一のデフォルトロケールルートを指し続けます。項目の設定全般は [ヘッダーナビゲーション](./header-navigation.mdx#versioned)を参照してください。
...
個別のアセット閲覧ページはリーフページであり、任意で有効にできるルートプレフィックス直下のページはアセット一覧です。どちらもデフォルトロケールにだけ存在し、ドキュメントのサイドバー、検索インデックス、`llms.txt`、サイトマップには掲載されません。そのため、日本語ドキュメントから参照した場合も、英語ドキュメントと同じデフォルトロケールのアセットルートが開きます。
