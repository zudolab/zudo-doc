...
`versioned: false` を指定すると、アーカイブ版のプレフィックスを加えず、単一のデフォルトロケールルートを指し続けます。項目の設定全般は [ヘッダーナビゲーション](./header-navigation.mdx#プロパティ-versioned)を参照してください。
...
個別のアセット閲覧ページはリーフページであり、任意で有効にできるルートプレフィックス直下のページはアセット一覧です。どちらもデフォルトロケールにだけ存在し、ドキュメントのサイドバー、検索インデックス、`llms.txt`、サイトマップには掲載されません。そのため、追加ロケールのドキュメントから参照した場合も、プライマリロケールのドキュメントと同じデフォルトロケールのアセットルートが開きます。
