...
zudo-docは**タイトトークン戦略**を採用しています。Tailwindのデフォルト値をすべてリセットし、小さく意図的なデザイントークンのセットに置き換えます。このページでは、スペーシング、タイポグラフィ、カラー、角丸、ブレークポイントの全体像を解説します。
...
zudo-docでは`src/styles/global.css`内でワイルドカードの`initial`リセットを使い、デフォルトを無効化しています:
...
リセット後は、明示的に定義されたトークンのみが機能します。リセットされたトークン（`p-4`や`bg-gray-500`など）を使っても効果はありません。値自体が存在しないためです。これにより**無効なトークンはビルドエラーやスタイル欠落として現れ、視覚的なバグにはなりません**。
...
Tailwindのすべてのデフォルト値は`initial`にリセットされています。プロジェクトで定義されたトークンのみが機能します。
</Warning>
...
```html
<!-- NG: Tailwindデフォルト — リセット済みで何も表示されない -->
