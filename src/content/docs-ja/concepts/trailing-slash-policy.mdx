...
zudo-docは「常に末尾スラッシュあり」のURLポリシーを採用しています。すべてのページは末尾が `/` で終わるパスに配置されます。例えば `/docs/getting-started` ではなく `/docs/getting-started/` です。この規則は、zudo-docがAstro時代に採用していた規約（zfb移行前に有効だった `trailingSlash: "always"` 設定）に基づいています。

歴史的に（Astro時代）、この適用は2箇所で行われていました：
