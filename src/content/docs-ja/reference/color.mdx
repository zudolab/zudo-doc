...
zudo-docは**3層カラー戦略**を採用し、サイト上のすべての色をテーマ切り替え可能にしています。Tailwindのデフォルトテーマはインポートされず、`preflight`と`utilities`のみが読み込まれるため、組み込みカラーは存在しません。`@theme`ブロックで定義されたプロジェクトトークンのみが機能します。これにより、カラースキームを切り替えるだけでサイト全体が一括で更新されます。
