...
| `accent` | `{ accent: 1 }` | 本文中のリンク、アクティブ状態・現在地の表示、CTA、クロームのリンクのホバー・フォーカス時の色 |
...
リンクの色は、リンクであること自体ではなく、置かれている場所で決まります：

- **本文中のリンク** — 文章の流れの中にあるリンク（`ContentLink`が描画するMarkdownのリンクや、バージョンバナー内のリンクなど）— は`text-accent underline`を使います。
- **クロームのリンク** — ナビのリスト、カード、チップ、ページャー — は`text-fg`（フッターの列、目次、パンくずリストといった補助的なクロームでは`text-muted`）のままにし、ホバー時・フォーカス時、または現在地を示すときにだけ`accent`に変わります。常時表示の下線は付けません。その代わり、目に見えるリスト、カードの枠、先頭のアイコンのいずれかで、リンクだと分かる構造を必ず持たせます。
- **補助的なクロームに並ぶインラインのリンク列** — ホームのヒーロー部分のリンク行と、フッターの著作権表記の行 — はその中間です。`text-fg`に常時の下線を付け、ホバー時に`accent`に変わります。

```html
<!-- Prose link -->
<a class="text-accent underline hover:text-accent-hover">Prose link</a>

<!-- Chrome link -->
<a class="text-fg hover:text-accent hover:underline focus-visible:text-accent focus-visible:underline">Chrome link</a>
```
...
ヘッダーのメニューで選ぶ **Light**、**Dark**、**System** は保存される設定です。ページで解決されるカラースキームはライトまたはダークのままです。デフォルトの `respectPrefersColorScheme: true` では、保存済みの設定がない初回訪問時に System が選ばれ、デバイスに従います。Light または Dark を選ぶと、利用者が再び System を選ぶまでその外観が固定されます。選択中の設定は `localStorage` の `zudo-doc-theme` に保存されます。

コンパクトメニューのアイコンは常に選択中の設定を表します。Light は太陽、Dark は月、System はモニターです。System を選んだ状態でデバイス設定がライトからダークに変わると、ページのカラースキームは変わりますが、選択アイコンはモニターのままです。`defaultMode` は `"light" | "dark"` のフォールバックです。`respectPrefersColorScheme: false` にすると、保存済みの選択がない初回訪問時にこの値が使われます。利用者は System を明示的に選ぶこともでき、その場合はデバイス設定に従います。

ライト／ダークの外観メニューなしで1つの固定スキームを使うには、`colorMode: false`を設定します。この場合は`colorScheme`がスキームを選びます：
