...
`ColorSchemeProvider`コンポーネント（`packages/zudo-doc/src/theme/color-scheme-provider.tsx`）がアクティブなカラースキームを読み取り、ビルド時に`:root`にCSSカスタムプロパティを注入します：
...
カラー文字列を繰り返す代わりにパレットインデックスを使用することで、重複を排除し、パレットカラーが変更された際（例：[Design Token Panel](./design-token-panel.mdx)経由）にセマンティックカラーが同期されるようになります。
...
- `background`、`foreground` — `ColorRef`（パレットインデックスまたはカラー文字列）
