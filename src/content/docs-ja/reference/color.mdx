...
zudo-docは**3層カラー戦略**を採用し、サイト上のすべての色をテーマ切り替え可能にしています。Tailwindのデフォルトテーマはインポートされず、`@theme`ブロックがカラー名前空間を`--color-*: initial`でリセットしてからプロジェクトトークンを定義します。プロジェクトトークンのみが機能するため、カラースキームを切り替えるだけでサイト全体が一括で更新されます。
