...
サーバーは設定可能なポート（デフォルト4322）で実行され、REST API経由でドキュメント履歴を配信します。標準の`pnpm dev`セットアップでは、zfbとdoc-history-serverが`run-p`を使用して同時に実行されます。
...
ルートの`pnpm dev`コマンドは、`run-p`を使用してzfb（ポート4321）とdoc-history-server（ポート4322）を同時に実行します。
