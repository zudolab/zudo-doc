...
カスタマイズはもう一段深いレイヤーで行います。共有ランプとセマンティックマップを `src/config/color-schemes.ts` で編集し（[Color](./color.mdx#カスタムカラースキームの追加) の「カスタムカラースキームの追加」を参照）、ソースにコミットする前に [Design Token Panel](./design-token-panel.mdx#aiワークフロー) の Palette タブで結果をライブプレビューできます（[カラースキームプレビュー](../guides/color-scheme-preview.mdx)を参照）。
