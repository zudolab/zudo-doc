...
各レンダラーは `@takazudo/zudo-doc/metainfo` の `FrontmatterCellRendererProps` を受け取る Preact コンポーネントです：
...
独自のレンダラーマップにキーを追加します。各値は`FrontmatterCellRendererProps`を受け取り、JSXまたは`null`を返すコンポーネント関数です：
