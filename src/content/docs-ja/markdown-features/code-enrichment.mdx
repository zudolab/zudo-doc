...
出力されるマークアップは `<div class="code-block-container"><div class="code-block-title">example.ts</div><pre class="hi-root"><code>…</code></pre></div>` です。タイトルのないフェンスは、コンテナもタイトルバーもない素の `<pre class="hi-root"><code>…</code></pre>` を出力します。
...
| トリガー | 出力される要素 | クラス名 |
| --- | --- | --- |
| `title="x"` 付きフェンス | `<div><div>title</div><pre><code></code></pre></div>` | `.code-block-container` > `.code-block-title` + `pre.hi-root` |
| タイトルなしフェンス | 素の `<pre><code></code></pre>` | `pre.hi-root` |
