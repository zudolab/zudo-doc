...
- `admonitionsPreset` のバリアント（`note`、`tip`、`info`、`warning`、`danger`、`details`）はオプトインの [Admonitions Preset](./admonitions-preset.mdx) 機能が提供します。Core レジストリ自体ではありません。レジストリはメカニズムであり、プリセットがコンポーネントマッピングを提供します。
