...
For a flat, ordered note tray, use [`NoteTrayIndex`](./note-tray-index.mdx) instead. It preserves tray ranks, dates, and the configured ascending or descending order, and offers index, card, and timeline presentations.
...
The `category` prop also accepts a nested category path. For example, `<CategoryNav category="changelog/core" />` lists the immediate release pages under `changelog/core/`, following the same pattern as `guides/layout-demos`. On the parent landing page, `<CategoryNav category="changelog" />` renders one card for each package subcategory's `index.mdx`.
...
The component's rendering logic ships from the package at `@takazudo/zudo-doc/category-nav`, and is wired into every MDX doc page automatically via the package-owned routes (`packageOwnedRoutes`, on by default) — there is no project file to maintain.
