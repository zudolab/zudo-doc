...
|------|------|----------|---------|-------------|
| `groupId` | string | No | — | Sync tab selection across groups with the same `groupId` (persisted in localStorage) |
...
| `value` | string | No | Value of `label` | Unique identifier for the tab |
| `default` | boolean | No | `false` | Set as the initially active tab |
