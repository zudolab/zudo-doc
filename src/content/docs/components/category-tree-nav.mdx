...
description: A server-rendered Preact component that renders a nested tree of child page links for category index pages.
sidebar_position: 9
---
...
| Nesting | Flat (children only) | Unlimited (`maxDepth` defaults to `Infinity`) |
...
The component's rendering logic ships from the package at `@takazudo/zudo-doc/category-tree-nav`, and is wired into every MDX doc page automatically via the package-owned routes (`packageOwnedRoutes`, on by default) — there is no project file to maintain.
