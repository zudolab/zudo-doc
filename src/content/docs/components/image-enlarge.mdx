---
title: Image Enlarge
description: Automatically adds a fullscreen-expand button to images that are wider than their rendered column.
sidebar_position: 10
---

When an image's intrinsic width is greater than its rendered width (accounting for device pixel ratio), a small expand button appears in the corner and the image gets a subtle border to signal interactivity. Clicking anywhere on the image — not just the corner button — opens it in a fullscreen dialog.

## Wide Image — Button Appears

The image below is 2400 px wide, so it is larger than the typical content column. The expand button becomes visible once the browser measures the rendered size.

![A wide placeholder image](/img/image-enlarge/image-wide.webp)

## Small Image — No Button

This image is only 300 px wide, which fits inside the column. The expand button stays hidden because the image is not larger than its container.

![A small placeholder image](/img/image-enlarge/image-small.webp)

## Opt-Out

Add `"no-enlarge"` as the Markdown title attribute to disable the expand button for a specific image, even if it is wide enough to qualify. The title attribute is also stripped from the rendered `<img>` so it never shows as a browser tooltip.

```md
![alt text](./image.png "no-enlarge")
```

![A wide image with opt-out applied](/img/image-enlarge/image-opt-out.webp "no-enlarge")

## Asset viewer caption links

When the asset viewer is enabled, a standalone image whose source is in the asset manifest gains an **Open asset page** link in its caption. The link opens the image's wide viewer page with dimensions, raw-file actions, background controls, and links back to referring documents. It is present even when that image is too small to show the enlarge button or uses `"no-enlarge"`.

See [Asset Viewer](../guides/asset-viewer.mdx) for a live example. Set `imageEnlarge: true` when you want the embedded image to offer the enlargement dialog as well as its asset-page link.

## How It Works

The feature has two parts:

- **Server render (SSR)** — the MDX paragraph (`p`) component override in zudo-doc's package MDX components (`packages/zudo-doc/src/mdx-components/`) wraps single-image paragraphs in a `<figure class="zd-enlargeable">` and injects a hidden `<button class="zd-enlarge-btn">`. Images mixed with surrounding inline text are not wrapped.
- **Runtime** — a Preact island (`ImageEnlarge`) uses `ResizeObserver` to compare `naturalWidth` against `clientWidth × devicePixelRatio` and toggles the button's `hidden` attribute accordingly. While that attribute is unset, the entire image becomes the click target and shows a hover affordance, so any click on the image opens it in a `<dialog>`. Press **ESC**, click the close button at the top-right of the viewport, or click outside the image to close it.

:::note

On high-DPI displays (e.g. 2× retina), eligibility accounts for device pixel ratio. A 2000 px image rendered at 1000 CSS px on a 2× screen is **not** eligible because `2000 ≤ 1000 × 2`.

:::

## Settings

Image enlarge is enabled via the `imageEnlarge` field in `zfb.config.ts`:

```ts title="zfb.config.ts"
export default defineConfig(
  zudoDoc({
    imageEnlarge: true,
  }),
);
```

Set `imageEnlarge: false` to disable the feature entirely.

## Customizing overlay colors

The enlarge button and the dialog's close button share a single token pair that controls their appearance:

- `image-overlay-bg` — bg layer color, rendered at 80% opacity behind the icon.
- `image-overlay-fg` — icon color, rendered at 100% opacity on top of the bg.

The right overlay color depends on the content photos, which vary between sites. The default values work for most cases, but any color scheme can override them per the three-tier color strategy (see [Color](../reference/color.mdx)):

```ts
// your colorSchemes override module, passed via the `colorSchemes` field in zfb.config.ts
semantic: {
  imageOverlayBg: { base: 4 },   // ramp stop — deep surface
  imageOverlayFg: { base: 0 },  // ramp stop — light text
  // or a literal OKLCH string: imageOverlayBg: "oklch(.185 .005 65)"
},
```

The Design Token Panel's Color tab (when enabled) exposes these tokens for live tweaking without editing code.

## See also

- [Image Enlarge (markdown feature)](../markdown-features/image-enlarge.mdx) — the `imageEnlarge` config knob and the paragraph-wrapping pipeline
