...
- **Server render (SSR)** — an MDX paragraph (`p`) component override in `pages/_mdx-components.ts` wraps single-image paragraphs in a `<figure class="zd-enlargeable">` and injects a hidden `<button class="zd-enlarge-btn">`. Images mixed with surrounding inline text are not wrapped.
