...
1. A trusted author opening `<title>` in the merged `head` remains caller-owned and authoritative; no generated title is added.
2. A nonblank visible `title` prop supplies the generated document title.
3. The merged localized/per-call `labels.preview` value supplies the generated document title.
4. The literal `Preview` value is used as the final fallback.
