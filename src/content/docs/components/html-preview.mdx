...
- Previews use `sandbox="allow-same-origin"` (or `sandbox="allow-scripts allow-same-origin"` when a `js` prop or `<script>` is present) so the iframe height auto-syncs via `contentDocument`. The `srcdoc` content is author-controlled MDX.
