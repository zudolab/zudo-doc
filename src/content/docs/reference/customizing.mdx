...
| **5. `chromeBindingsModule`** | Injecting host content/callables into injected routes, including [custom MDX components](../guides/custom-components.mdx#global-registration-mdxextras) | One host module plus stub wiring |
...
- **Chrome-rendered:** `header`, `footer`, `breadcrumb`, `toc`, `sidebar`, `theme-toggle`, `page-loading`, `doc-pager`, `sidebar-tree-island`, `sidebar-toggle-island`, `desktop-sidebar-toggle-island`, `image-enlarge`, `doc-history`, and `site-tree-nav-island`. The CLI can copy these sources, but a fresh scaffold has no host call site for them, so the copy is a functional no-op: editing it does not change the rendered chrome.
...
**Limit:** an ejected copy no longer receives package updates for that component — you take on its maintenance. The [CLI warning work](https://github.com/zudolab/zudo-doc/issues/2717) tracks clearer guidance for chrome copies; [the chrome-markup seam discussion](https://github.com/zudolab/zudo-doc/issues/2719) tracks the missing supported customization path. Today, there is no supported way to customize chrome markup through ejection.
...
A fresh scaffold needs one more change before this example can affect its doc pages. Its self-contained `pages/docs/[[...slug]].tsx` stub shadows the injected route and calls `createChrome(routeCtx)` without bindings. Statically import `chromeBindings` into that stub and pass it to `createChrome()`; apply the equivalent change to locale or versioned stubs that your project uses. A scaffold generated after [the stub-threading fix](https://github.com/zudolab/zudo-doc/issues/2716) may already contain the presentational wiring, but the [custom components guide](../guides/custom-components.mdx#global-registration-mdxextras) is the complete, universal recipe.
...
See the [Deployment guide](../guides/deployment.mdx) and the [AI Assistant API reference](./ai-assistant-api.mdx) for the full runbook (secrets, KV, CORS, rate limiting).
...
- [Custom Components](../guides/custom-components.mdx) — register project-owned MDX components and thread bindings through route stubs
...
- [Migrating from v3](../getting-started/migrating-from-v3.mdx) — moving an existing generated project onto this shape
