...
| **6. `chromeBindingsModule`** | Replacing primary chrome or injecting host callables, including [custom MDX components](../guides/custom-components.mdx#global-registration-mdxextras) | One host module |
...
Now pages can declare `tier: core` in frontmatter and it validates at build time. Combine this with [`docContentHeaderExtras`](./host-chrome-bindings.mdx) (rung 6) to actually render a badge from that key.
...
Every pack defines both its light and dark values, so readers can keep choosing Light, Dark, or System from the appearance menu on any pack. System follows the device independently of the active pack. Add `themePackSwitcher: true` to let readers switch packs live from a bottom-right flyout.

**Reach for it when:** you want a different overall look and a bundled pack matches it.

**Limit:** you pick from installed packs as-is; adjusting individual tokens on top is the next rung. See [Theme Packs](./theme-packs.mdx).
...
If your project also merges a second design system’s `@theme`, read [Coexisting with another `@theme`](./design-system.mdx#coexisting-with-another-theme).
...
**A different kind of eject — `zudo-doc eject logo`.** This subcommand shares the `eject` name but is not a component-source copy like the targets above: it never touches `.zudo-doc.json` provenance or a component directory. It is an **asset materialization** — it renders the generated auto-logo to a real SVG file and rewires the [`logo`](../guides/configuration.mdx#identity-and-urls-logo) config field to point at it, so you end up owning an editable file instead of the request-time generated default.
...
The scaffold stubs are special. `pages/index.tsx` shadows nothing: `/` is never injected, so it is a one-line re-export that provides the package home page. `pages/docs/[[...slug]].tsx` does shadow the injected doc route, but it remains an explicit host-owned seam rather than an optional customization example. zfb 2.13.1 renders injected dynamic routes in both dev and build; the kept stub preserves host ownership and customization. See [Routing Conventions](../develop/routing-conventions.mdx#package-route-injection-and-shadowing) for the complete injection and shadowing model.
...
See the showcase [Deployment guide](../guides/deployment.mdx) and [AI Assistant API reference](./ai-assistant-api.mdx) as a host-implementation reference (secrets, soft per-IP KV, exact UTC-day admission, CORS, and preview limitations). Binding the named class without supplying its source is not sufficient.
...
- [Theme Packs](./theme-packs.mdx) — the bundled packs, the switcher UI, and the theme CLI
