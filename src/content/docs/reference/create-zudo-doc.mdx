...
- **No `check:html` / HTML-validation** step and **no `gen:z-index` / `check:z-index`** codegen — both were pruned from generated projects. The default z-index tiers ship from the package; HTML validation and pre-push suites are opt-in add-backs (see [Customizing → Restoring pre-push and HTML validation](./customizing.mdx#rung-8-—-restoring-pre-push-and-html-validation)).
