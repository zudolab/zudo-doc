...
| `./sub/ref-doc/` | `./sub/ref-doc` | `ref-doc` |
| `../ref-doc` | `../ref-doc` | `ref-doc` |
| `/tmp/scratch/ref-doc` | `/tmp/scratch/ref-doc` | `ref-doc` |
...
`@takazudo/zdtp` is added **only when you enable `designTokenPanel`**. It is an *optional* peer of `@takazudo/zudo-doc`, so a project that leaves the panel off never installs it. The package's chrome still imports the panel bootstrap *module* at module scope (so zfb's island scanner can find it), but that module reaches `@takazudo/zdtp` solely through a rejection-handled dynamic `import()` on first use (see [Lazy loading](./design-token-panel.mdx#implementation-lazy-loading)) — an edge the bundler tolerates when the package is absent, so a panel-less project builds without it. If you turn `designTokenPanel` on in a project that was scaffolded without it, install `@takazudo/zdtp` yourself and add `@import "@takazudo/zdtp/styles.css";` to `src/styles/global.css`.
...
Because `--respect-system-preference` defaults to `true`, a first visit without a saved choice selects System and follows the device. `defaultMode` remains the light/dark fallback when system following is disabled; see the [Color reference](./color.mdx) for persistence and the appearance menu.

### Theme Pack

| Flag | Description | Default |
|------|-------------|---------|
| `--theme-pack <slug>` | Theme pack applied to the generated project — any slug from the [Theme Gallery](./theme-gallery.mdx) | `default` |
