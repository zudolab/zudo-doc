...
The actual colors are **not fixed** — each color scheme defines its own values for each slot. The names are conventions, not guarantees. That's why zudo-doc uses abstract numbered tokens (`p0`–`p15`) instead of color names.
...
- `background`, `foreground` — base color strings
