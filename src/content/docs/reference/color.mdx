...
zudo-doc uses a **three-tier color strategy** to keep every color on the site themeable. The default Tailwind theme is not imported — only `preflight` and `utilities` are loaded, so no built-in colors exist. Only project tokens defined in the `@theme` block work. This ensures switching a color scheme updates the entire site at once.
...
```css
/* WRONG — don't reference component-scoped variables */
.my-component {
