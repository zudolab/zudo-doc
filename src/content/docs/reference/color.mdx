...
To change the active color scheme, set `colorScheme` in `zfb.config.ts`:
...
`colorSchemes` is one of `zudoDoc()`'s escape hatches — when omitted, the two packaged schemes above are used as-is. To add or replace a scheme, write your own module (e.g. `src/config/color-schemes.ts`) building a `Record<string, ColorScheme>`, then pass it through the `colorSchemes` field in `zfb.config.ts`. A new scheme needs its own `ramps` and a `map` for each mode you want (spread `SEMANTIC_RAMP_DEFAULTS` and override only what needs to differ). Spread in `defaultColorSchemes` too if you want to keep the two packaged schemes alongside your own:
