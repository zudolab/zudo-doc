...
|------|------|---------|------------|
| 1 | **Ramps** | Shared OKLCH color ramps a scheme is built from | `src/config/color-schemes.ts` → `ColorSchemeProvider` → `:root` |
...
- **Retune a ramp stop** (edit `src/config/color-schemes.ts`) → every semantic role built from it updates
...
zudo-doc's color engine is **ramp-native**: a `ColorScheme` is `{ ramps, map }` (see `src/config/color-schemes.ts`).
...
**Default Light** and **Default Dark** — the only two bundled schemes — share the exact same `base`/`accent`/`state` ramp values (one `ramps` object in `src/config/color-schemes.ts`); only the per-mode `map` differs between them.
...
The 23 semantic roles are the color surface components actually use. In `src/styles/global.css`, the `@theme` block maps them (plus the 4 base roles) into Tailwind-compatible tokens:
...
Both schemes wire all 23 roles; the table below shows Default Dark's `map.semantic` — the authored reference scheme in `src/config/color-schemes.ts`. Default Light re-points several of these to different ramp stops or per-mode literals; see [Per-mode literal overrides](#per-mode-literal-overrides).
...
| `mermaidNodeBg` / `mermaidText` / `mermaidLine` / `mermaidLabelBg` / `mermaidNoteBg` | see `color-schemes.ts` | Mermaid diagram colors |
...
Edit `map.semantic` in `src/config/color-schemes.ts`. A role can point at a shared ramp stop or a literal OKLCH string:
...
zudo-doc ships exactly two color schemes — **Default Light** and **Default Dark** — sharing one set of ramps (see `src/config/color-schemes.ts`). Both are hand-authored and AA-tuned in place. There is no bundled catalog of community or terminal presets to pick from; an earlier iteration of the Design Token Panel had a "Scheme…" dropdown for browsing 50+ bundled presets, but the ramp-native model dropped it — see [Design Token Panel](./design-token-panel.mdx).

## Adding a Custom Color Scheme

Add a new entry to the `colorSchemes` object in `src/config/color-schemes.ts`. A new scheme needs its own `ramps` and a `map` for each mode you want (spread `SEMANTIC_RAMP_DEFAULTS` and override only what needs to differ):
...
After adding or tweaking a scheme, run `pnpm contrast:audit` to check every pair in the WCAG contrast matrix — see the `color-scheme-a11y` skill for the full methodology and thresholds.
