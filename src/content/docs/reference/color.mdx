...
Each color scheme can override the default palette-slot mapping via the `semantic` property in `src/config/color-schemes.ts`:

```ts
"My Custom Scheme": {
  // ...palette, background, foreground...
  semantic: {
    accent: "#d33682",     // override default cyan with magenta
    accentHover: "#6c71c4",
    surface: "#f4efdd",
  },
},
