...
<Warning title="Requires a preview-data binding">

`buildFrontmatterPreviewEntries` defaults to a no-op. Point
`chromeBindingsModule` at a module that binds it. Fresh route stubs already
consume the module; do not edit them. See [Custom Components](../guides/custom-components.mdx)
and [Host Chrome Bindings](./host-chrome-bindings.mdx).
