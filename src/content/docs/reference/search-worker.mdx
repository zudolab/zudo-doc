...
The Search Worker is a sub-package at `packages/search-worker/` that deploys as a Cloudflare Worker. It uses the [MiniSearch](https://lucaong.github.io/minisearch/) engine and consumes the same `search-index.json` search index as the built-in client-side search, but runs server-side on Cloudflare Workers runtime. The two share the index, not the search engine — the built-in client-side search is a custom word-match scorer, not MiniSearch.
...
The Search Worker is an **additive option**, not a replacement. The primary search experience remains the built-in client-side search (a custom word-match scorer) via the search dialog (`Ctrl+K` / `Cmd+K`). See [When to Use Each](#when-to-use-each) for guidance.
...
The Search Worker uses **wildcard CORS** — every response sends `Access-Control-Allow-Origin: *`, so any origin can call it. This is intentionally **different from the [AI Assistant API](./ai-assistant-api.mdx), which uses a per-origin allowlist** (`aiChatAllowedOrigins`). The Search Worker is an unmetered, opt-in service with no per-call cost to gate, so it does not restrict origins. Do not assume the two endpoints share a CORS policy.
...
| Search engine              | Custom word-match scorer      | MiniSearch                 |
| Search config              | Weighted substring match (title 3 / description 2 / body 1) | `prefix: true, fuzzy: 0.2, boost: { title: 3, description: 2 }` |

Both read the same search index (`search-index.json`), but they do not share a search engine — the built-in client-side search scores plain substring matches, while the Search Worker queries the index with MiniSearch's prefix/fuzzy matching.
