...
Their selectors name a desktop AND a mobile surface, but only the **desktop** half is delivered today: the mobile sidebar drawer and mobile TOC are zfb client islands, and zfb strips bare `data-*` attributes from island roots during SSR, so `aside[data-zd-mobile-sidebar]` / `div[data-zd-mobile-toc]` never actually matches in the rendered page. Instead, each mobile surface inherits the font-family resolved on its real DOM ancestor — the drawer renders inside `header[data-header]` (so it follows `--zdc-header-font`, not `--zdc-sidebar-font`), and the mobile TOC renders inside `.zd-content` (so it follows `--zdc-doc-prose-font`, not `--zdc-toc-font`). Every bundled pack leaves those ancestor tokens at their seam defaults, so this is invisible today — but a pack that gives the header or the prose area its own distinct font, independent of `--zdc-sidebar-font`/`--zdc-toc-font`, will see the mobile drawer/TOC follow that ancestor's font instead of the token it actually set. Tracked as [zudolab/zudo-doc#2898](https://github.com/zudolab/zudo-doc/issues/2898). `--zdc-header-font` is unaffected — its selector, `header[data-header]`, is not an island.
