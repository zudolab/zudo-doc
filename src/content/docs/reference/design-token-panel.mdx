...
See [Color Scheme Preview](../guides/color-scheme-preview.mdx) for a walkthrough.
...
- **A resolved light/dark appearance change re-seeds the tab's ref/literal defaults** to the new mode's wiring (e.g. `bg` flips `base-0` ↔ `base-4`) — this includes an OS change while System is selected. See [Implementation](#implementation) for the destroy+reconfigure mechanism behind this.
- **Palette, Spacing, Font, and Size tweaks carry over appearance changes untouched.** Those tabs are mode-independent, so a ramp-curve edit or a spacing change survives light↔dark exactly as before.
- **A saved Color-tab override is scheme- and mode-scoped.** The package-default builder pairs `Default Light` and `Default Dark` in `panelSettings.colorMode`, so a saved Color override is restored only for its active scheme identity. An override made in one mode does not repaint the other; the Palette, Spacing, Font, and Size tabs remain shared across appearance changes.
...
- **Scale** — the raw `--text-scale-*` steps (`2xs` → `2xl`), edited as numeric values with CSS-value sanitization.
- **Font size** — the semantic roles (`text-micro` … `text-display`). This is a *reference* tier: each row picks a **Scale** step rather than a raw value, so editing a step propagates to every role pointing at it, exactly mirroring how the Color tab's semantic rows point at ramp stops.
- **Line height**, **Font weight**, **Font family** — the remaining typography tokens. The family fields accept any valid CSS `font-family` value, including multi-stack fallbacks; invalid values (unbalanced quotes, raw JavaScript, etc.) are rejected on change so pasted snippets cannot break the page.
...
- **Radius** — `--radius-DEFAULT`, `--radius-lg`, and `--radius-full`. The last carries a pill toggle that switches between its `9999px` sentinel and an editable value.
- **Transition** — the default transition duration.
...
- **Dock modes.** Float, right dock, bottom dock, and a collapsed mini pill, switchable with `Alt+1` … `Alt+4`. The two docks reserve space in the host document and release it when the panel closes, so the page reflows beside the panel instead of disappearing behind it. The chosen mode persists across page loads.
- **Ghost when idle.** Fades the panel while you are not interacting with it, so a color change can be judged against an unobstructed page.
...
- **Token search and command palette.** Fuzzy search across every tab with per-tab hit counts, and a `Cmd/Ctrl+K` palette for jumping straight to a token.
- **Changed-token state.** Edited rows are marked, each tab shows a badge, and the footer summarises the total ("*N* tokens changed across *M* tabs"). **Changed only** filters the view down to those rows, **Copy diff** puts them on the clipboard, and **Revert all** puts every one of them back. The Font, Spacing, and Size tabs each carry their own **Reset** action as well.
...
5. The panel validates, merges, and applies the new tokens instantly. Hit **Revert all** in the changed-token toolbar to roll everything back if you don't like the result.
...
Changing the resolved light/dark appearance does **not** delete your saved tweaks, and an open panel stays open across the change. The appearance menu dispatches `color-scheme-changed` only when the resolved appearance changes; the host destroys and reconfigures the panel for the new mode (see [Implementation](#implementation)) rather than patching it in place, so the Color tab's ref/literal *defaults* follow the newly active mode and any saved Color override is restored for that mode's scheme identity. Choosing System while the device already matches the current appearance changes the saved preference without rebuilding the panel. Palette, Spacing, Font, and Size tweaks carry over untouched. See [Mode-scoped editing model](#the-tab-strip-color-mode-scoped-editing-model).

<Note title="Storage prefix continuity guarantee">

The storage prefix `zudo-doc-tweak` is locked in the package-default builder (`@takazudo/zudo-doc/design-token-panel-config`, which this showcase consumes directly) and will not change — so tweaks saved by an earlier release carry over untouched. Persistence details beyond that prefix belong to `@takazudo/zdtp`, not to a repository-owned zudo-doc compatibility layer.
...
The bootstrap reaches zdtp only through the package-owned `@takazudo/zudo-doc/zdtp-loader` subpath. The preset installs a throwing stub for that one specifier whenever zdtp is not bundled — resolved as `bundleZdtp ?? designTokenPanel` — so a project that never sets `bundleZdtp` keeps exactly today's behavior: panel off means no zdtp chunks at all. Setting `bundleZdtp: true` keeps the real loader even with the panel off; see [Mounting your own panel](#implementation-mounting-your-own-panel) for why you'd want that. Your own `import "@takazudo/zdtp"` statements are not affected either way.
...
On every `color-scheme-changed` event (dispatched when the resolved appearance changes, including an OS change while System is selected), the bootstrap reads the new mode and coalesces rapid changes onto a single macrotask, then calls `handle.destroy()` followed by `configurePanel(buildDesignTokenPanelConfig(mode))` — zdtp's `configurePanel` throws if re-run in place with a structurally different config, so destroy-then-reconfigure is the only supported way to swap the Color tab's mode-scoped defaults. A preference change that leaves the resolved appearance unchanged uses `theme-preference-changed` and does not rebuild the panel. If the panel was open, it's re-mounted open.
...
### Mounting your own panel

`designTokenPanel` and `bundleZdtp` control two different things, and conflating them is exactly what caused zudo-doc#4261: `designTokenPanel` controls **mounting** — whether the package's own `DesignTokenPanelBootstrap` island renders; `bundleZdtp` controls **bundling** — whether `@takazudo/zdtp` ships in the build at all. They default to tracking each other (see [Lazy loading](#implementation-lazy-loading)), but a host that wants to mount its own panel island — instead of the package's — needs to set them independently.

Call the same bootstrap the package island calls, from your own `"use client"` island — and call it through `runDesignTokenPanelBootstrapOnce`, not `bootstrapDesignTokenPanel` directly. The once-latch is what keeps a re-render or a soft navigation from bootstrapping a second time; a second bootstrap binds a second `toggle-design-token-panel` listener, so one trigger click counts as two toggle intents and the panel never opens.
...
export function HostPanelBootstrap() {
  runDesignTokenPanelBootstrapOnce(buildDesignTokenPanelConfig);
  return null;
}
```

Wire that component into the `BodyEndIslands` slot of your `chromeBindings` module, wrapped in `Island({ when: "load" })` — the same shape the package's own panel island uses. It renders nothing on either side; `bootstrapDesignTokenPanel` no-ops during SSR through its own `typeof window === "undefined"` guard.

Two settings move together to make this work:

- `designTokenPanel: false` — so the package mounts nothing of its own; only your island renders a panel.
- `bundleZdtp: true` — so the loader stays real. Leave it unset with the panel off and `@takazudo/zudo-doc/zdtp-loader` shadows to a throwing stub; your bootstrap's dynamic import rejects the moment the panel is opened, even though the build itself went green.

You also own the trigger. `designTokenPanel: false` drops the built-in `design-token-panel` trigger from `settings.headerRightItems` (the package refuses to render a button that would dispatch into a document with no panel island of its own), so a host-mounted panel needs its own button — a `headerRightItems` entry resolved through `headerRightComponents`, or any element of yours — dispatching `toggle-design-token-panel` on `window`.

You also install `@takazudo/zdtp` yourself (it's an optional peer dependency of `@takazudo/zudo-doc`) and import its stylesheet — `@import "@takazudo/zdtp/styles.css"` — in your own `global.css`. Nothing adds that line for you here: `create-zudo-doc` writes it only when you scaffold a project with `designTokenPanel` on.

A missing `@takazudo/zdtp` with `bundleZdtp: true` fails the build resolving `@takazudo/zudo-doc/zdtp-loader`'s static re-export — loud, by design, rather than a silent runtime failure.

<Warning title="Invalid combination">

`designTokenPanel: true` together with `bundleZdtp: false` is rejected at config resolution: a mounted package panel with a stubbed loader is guaranteed to throw the moment it's opened, so zudo-doc fails the build instead of shipping it.
...
|---|---|---|
| `designTokenPanel` | `boolean` | Mounts the package's own panel island. Default `false`. |
| `bundleZdtp` | `boolean` | Bundles `@takazudo/zdtp` into the build. Default `undefined` (follows `designTokenPanel`: `bundleZdtp ?? designTokenPanel`). `true` with `designTokenPanel: false` bundles zdtp without mounting the package panel — the combination for mounting your own panel (see [Mounting your own panel](#implementation-mounting-your-own-panel)). `false` with `designTokenPanel: true` is rejected at config resolution. |
