...
- The FOUC-prevention script injected into `<head>` by the doc layout (`@zudo-doc/zudo-doc-v2/doclayout`) performs the same v2-then-v1 lookup so saved colors apply before first paint on old devices too.
...
The storage prefix `zudo-doc-tweak` is locked in `src/config/design-token-panel-config.ts` and will not change across the zdtp migration. Any tokens a user saved before the migration continue to load automatically — no manual migration step is needed.

</Note>

## Implementation

The panel UI is provided by the `@takazudo/zudo-design-token-panel` (zdtp) npm package. zudo-doc wires it via `configurePanel(designTokenPanelConfig)` in `src/lib/design-token-panel-bootstrap.ts`. The panel self-mounts as a side-effect of that call — there is no Preact island registration for it.
