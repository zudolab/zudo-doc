...
Call the same bootstrap the package island calls, from your own island:
...
You also install `@takazudo/zdtp` yourself (it's an optional peer dependency of `@takazudo/zudo-doc`) and import its stylesheet — `@import "@takazudo/zdtp/styles.css"` — in your own `global.css`; the package only does this automatically when `designTokenPanel` is on.
...
| `bundleZdtp` | `boolean` | Bundles `@takazudo/zdtp` into the build. Default `undefined` (follows `designTokenPanel`: `bundleZdtp ?? designTokenPanel`). Set to `true` with `designTokenPanel: false` to mount your own panel (see [Mounting your own panel](#implementation-mounting-your-own-panel)); `true` with `designTokenPanel: false` bundles zdtp without mounting the package panel, `false` with `designTokenPanel: true` is rejected at config resolution. |
