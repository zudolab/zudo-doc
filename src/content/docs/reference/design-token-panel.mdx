...
**Config-level per-mode literal defaults aren't possible.** `TierItem.default` is a single string in zdtp's config contract, so an AA-tuned per-mode literal (like `danger`) is seeded from whichever mode is active at reconfigure time rather than declared as a `{light, dark}` pair up front. Runtime per-mode editing (the **Per-mode** checkbox above) is unaffected — this only limits what a *config* can declare as a starting default.
