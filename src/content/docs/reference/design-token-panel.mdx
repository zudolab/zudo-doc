...
- **A saved Color-tab override is mode-agnostic, not mode-scoped.** zdtp persists the whole 27-role semantic map as one flat block rather than keying it per scheme, so an override made in one mode repaints in the *other* mode too, until you click **Reset all**. This is a documented interim limitation, not a bug — per-mode (scheme-keyed) persistence is tracked upstream as [zdtp#500](https://github.com/Takazudo/zudo-design-token-panel/issues/500) (relates to [zdtp#343](https://github.com/Takazudo/zudo-design-token-panel/issues/343)).
