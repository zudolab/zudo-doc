...
This showcase imports the package-default builder (`@takazudo/zudo-doc/design-token-panel-config`) directly into its own bootstrap island (`src/components/design-token-panel-bootstrap.tsx` → `src/lib/design-token-panel-bootstrap.ts`, see [Implementation](#implementation)) rather than going through `designTokenPanelConfigModule` — it used to keep a byte-for-byte-mirrored copy of the builder at `src/config/design-token-panel-config.ts`, retired as redundant. Use `designTokenPanelConfigModule` when you want the package's own auto-injected bootstrap (the default body-end island under `packageOwnedRoutes`) to pick up your custom builder instead of hand-rolling a bootstrap island of your own.
...
- **A saved Color-tab override is mode-agnostic, not mode-scoped.** An override made in one mode repaints in the *other* mode too, until you click **Reset all** — only the *defaults* are mode-faithful. This is a deliberate consequence of this showcase's **scheme-less** color config, not a zdtp limitation: zdtp 0.4.5 does ship per-scheme, per-mode keyed color persistence ([zdtp#500](https://github.com/Takazudo/zudo-design-token-panel/issues/500)), but this site runs a single scheme-less cluster (`colorSchemes = {}`, no `colorMode`) and switches light/dark externally, so zdtp resolves one shared scheme identity and a saved override applies to both modes by design.
...
- The FOUC-prevention script injected into `<head>` by the doc layout (`@takazudo/zudo-doc/doclayout`) performs the same lookup so saved tokens apply before first paint on old devices too.
- Clicking **Reset all** in the panel header clears the v3 envelope (and any lingering legacy keys), restoring the color scheme declared by the `colorScheme` field in `zfb.config.ts`.

Switching the light/dark theme does **not** delete your saved tweaks, and an open panel stays open across the toggle. The theme toggle dispatches a `color-scheme-changed` event; the host destroys and reconfigures the panel for the new mode (see [Implementation](#implementation)) rather than patching it in place, so the Color tab's ref/literal *defaults* follow the newly active mode while every tweak on Palette, Spacing, Font, and Size — plus any saved Color-tab *override* — carries over untouched. See [Mode-scoped editing model](#the-five-tabs-color-mode-scoped-editing-model) for the override-carries-across-modes caveat.

<Note title="Storage prefix continuity guarantee">

The storage prefix `zudo-doc-tweak` is locked in the package-default builder (`@takazudo/zudo-doc/design-token-panel-config`, which this showcase consumes directly) and will not change. Any tokens a user saved under an older key version continue to load automatically — they are migrated into the current `zudo-doc-tweak-state-v3` envelope on first load, with no manual migration step needed.

</Note>

## Implementation

The panel UI is provided by the `@takazudo/zdtp` (zdtp) npm package. The wiring mechanism — calling `configurePanel`, hooking zfb's navigation lifecycle, and the mode-scoped destroy+reconfigure described below — lives in `@takazudo/zudo-doc/design-token-panel-bootstrap`. The project's `src/lib/design-token-panel-bootstrap.ts` passes that mechanism the panel's config *data* by calling `bootstrapDesignTokenPanel(buildDesignTokenPanelConfig)`, where `buildDesignTokenPanelConfig` (imported directly from `@takazudo/zudo-doc/design-token-panel-config`, the package-default builder) is a `(mode) => PanelConfig` builder, not a plain config object — this is what lets the Color tab follow light/dark. The panel self-mounts as a side-effect of that call — there is no Preact island registration for it.
...
The tokens themselves — spacing scale, font scale, icon sizes, palette — are still configured in `src/styles/global.css` (`@theme`) and, for the color palette, either the package's `@takazudo/zudo-doc/color-schemes-defaults` or your own `colorSchemes` override (see [Color](./color.mdx)). The panel only tweaks the in-browser copy of those tokens; it never writes to source files.
