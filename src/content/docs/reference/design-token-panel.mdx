...
The Color tab shows the 23 resolved semantic roles (`accent`, `codeBg`, `success`, `danger`, `mermaidText`, `chatAssistantBg`, …) as direct OKLCH swatches — one per `--zd-*` custom property. Editing a swatch here overrides that role directly, independent of which ramp stop it normally resolves to.

#### Matched-keyword tokens

Two semantic color tokens drive how matched keywords are highlighted. They live alongside the other semantic swatches in the Color tab and are consumed by the search widget at `pages/lib/_search-widget.tsx` to style hits in the search UI.
...
The panel UI is provided by the `@takazudo/zdtp` (zdtp) npm package. zudo-doc wires it via `configurePanel(designTokenPanelConfig)` in `src/lib/design-token-panel-bootstrap.ts`. The panel self-mounts as a side-effect of that call — there is no Preact island registration for it.
