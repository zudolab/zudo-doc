...
The server runs on a configurable port (default 4322) and serves document history via a REST API. A project scaffolded with `docHistory` enabled gets this wired automatically: `create-zudo-doc` emits a `dev` script of `run-parallel dev:zfb dev:history`, so zfb and the doc-history-server run concurrently. `run-parallel` is a small bin shipped by `@takazudo/zudo-doc`, so this needs no extra dev dependency.
...
When additional locales are configured, `create-zudo-doc` appends one
`--locale <lang>:src/content/docs-<lang>` flag for every configured locale, in
order. For example, EN + JA + DE emits `--locale ja:src/content/docs-ja
--locale de:src/content/docs-de`. To run the server manually with other flags,
see [Server Options](#server-mode-local-development-server-options) below.
...
A `docHistory`-enabled project's generated `dev` script (`run-parallel dev:zfb dev:history`) runs both zfb (port 4321) and the doc-history-server (port 4322) concurrently.
