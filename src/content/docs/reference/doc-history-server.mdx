...
The Doc History Server is a sub-package at `packages/doc-history-server/` that handles all git history extraction for the [Document History](/docs/guides/doc-history) feature. It was extracted from the Astro build pipeline to decouple expensive git operations from the site build.
