...
That module's `chromeBindings` export should be built with `defineChromeBindings`, exported from `@takazudo/zudo-doc/chrome-bindings`, rather than a plain object literal typed `ChromeHostBindings`. Its input type — `ChromeBindingsInput` — declares the exact props or args each slot's real call site passes, so a component that requires a prop the chrome never provides is a compile error (the drift-detection check from #2674); the wide `ChromeHostBindings` shape the routes plugin needs is produced by one internal widening step you never have to reason about.
