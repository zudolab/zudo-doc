...
<Warning title="Fresh scaffold doc-route stubs need threading">

A fresh scaffold includes self-contained document-route stubs. They shadow the injected routes and call `createChrome(routeCtx)` without bindings, so `chromeBindingsModule` does not reach document pages until you edit those stubs. Follow the [global `mdxExtras` registration pattern](../guides/custom-components.mdx#global-registration-mdxextras) to thread the bindings through every relevant stub. The scaffold-template fix is tracked in [#2716](https://github.com/zudolab/zudo-doc/issues/2716).
