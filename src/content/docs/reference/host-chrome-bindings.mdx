...
| `DesignTokenPanelBootstrap` | Real package bootstrap; it mounts when `designTokenPanel: true`, with no host wiring. This slot only replaces which builder mounts while the package panel is ON — it stays unmounted with `designTokenPanel: false` regardless of what you pass here. To mount your OWN panel with the package panel off, call `bootstrapDesignTokenPanel` from your own island (e.g. wired through the `BodyEndIslands` slot below) and set `bundleZdtp: true`, or the real loader stays shadowed by a throwing stub. See [Mounting your own panel](./design-token-panel.mdx#implementation-mounting-your-own-panel). |
...
### `sidebarNodes` is a `createHeaderWithDefaults` prop, not a `Header` binding slot

The `Header` row above replaces the whole component: your replacement receives `HeaderSlotProps` and owns its own mobile-drawer tree entirely. `sidebarNodes` is a different, narrower seam — an optional prop on `HeaderWithDefaultsProps` that overrides only the tree the *package's* header factory passes to `SidebarToggle`, without replacing the rest of the header. It is available on `createHeaderWithDefaults(ctx)` and on `createChrome(...).HeaderWithDefaults`; it is **not** part of `HeaderSlotProps`, so a `chromeBindingsModule`-configured `Header` replacement never receives it. See `createHeaderWithDefaults(ctx)` in `API.md` for the full contract.
...
For paragraphs, lists or a longer introduction, use [home.introMarkdown](../guides/configuration.mdx#home-page-home). It renders a separate compact prose section below the identity; `homeExtras` and `extras` retain their inline navigation semantics.
