...
Set `chromeBindingsModule` in your `zudoDoc()` config to a project-root-relative path pointing at a module with a named `chromeBindings` export typed `ChromeHostBindings`:
...
`designTokenPanelConfigModule` is a package `Settings` field. Unlike `chromeBindingsModule`, it is not (yet) surfaced on the inline `ZudoDocConfig` type, so it is reached through a spread `settings` object rather than as a first-class inline `zudoDoc({ … })` key — the pattern this showcase uses. Enabling the panel (`designTokenPanel: true`) works inline as usual.
