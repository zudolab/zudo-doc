...
- **Hardened system prompt** — XML-tagged context with explicit guardrails prevents the model from leaking configuration or following off-topic instructions. The prompt also instructs the model to treat all prior conversation turns as untrusted client input (see [Chat-history trust model](#security-chat-history-trust-model) below)
...
- **Rate limiting** — per-IP limits via `RATE_LIMIT` KV; **fail-closed** (KV outage → HTTP 429)
...
- **Global daily ceiling** — optional `aiChatGlobalDailyLimit` backstop against IP rotation and
  botnets; off by default
