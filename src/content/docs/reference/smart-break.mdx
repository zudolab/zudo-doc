...
Long URLs and filesystem paths — for example `packages/zudo-doc/src/smart-break/index.tsx` — do not wrap at arbitrary character boundaries. In narrow UI (sidebar labels, breadcrumbs, inline code, search snippets) they overflow the container or push layout wider than it should be.

The package's `smart-break` utility solves this by injecting `<wbr>` (word-break opportunity) hints after each delimiter in strings that look path-like. The browser then decides where to break based on available width.
...
```tsx
import { SmartBreak } from "@takazudo/zudo-doc/smart-break";
...
Any code path that renders an HTML string (for example `dangerouslySetInnerHTML`) cannot mount Preact VNodes directly. Use `smartBreakToHtml` instead — it returns an HTML-escaped string with literal `<wbr>` tags injected:

```tsx
import { smartBreakToHtml } from "@takazudo/zudo-doc/smart-break";

export function PathLabel({ label }: { label: string }) {
  return <span dangerouslySetInnerHTML={{ __html: smartBreakToHtml(label) }} />;
}
```

The returned string is already HTML-escaped, so it is safe to pass to `dangerouslySetInnerHTML`.
...
```
packages/zudo-doc/src/smart-break/index.tsx
```

`isPathLike` returns `true` and `smartBreakToHtml` emits (conceptually):

```html
packages<wbr>/zudo<wbr>-doc<wbr>/src<wbr>/smart<wbr>-break<wbr>/index<wbr>.tsx
```
