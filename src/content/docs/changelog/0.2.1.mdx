...
Workaround until you are on a zfb release with the forward fix (below): keep a local, scanner-visible pass-through shim and point the header import at it.

```tsx
// src/components/theme-toggle.tsx
"use client";
export { ThemeToggle } from "@takazudo/zudo-doc/theme-toggle";
```
...
0.2.1 stopped shipping local copies of several modules that the package now owns: the rehype plugins, hast-utils, url-utils, docs-source-map, use-active-heading, toc, theme-toggle, sidebar-resizer, and the doc-history util. After you sync the templates, nothing references them — delete them. (The zudo-css-wisdom upgrade removed 13 such files.)
