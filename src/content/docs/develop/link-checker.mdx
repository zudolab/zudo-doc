...
The link checker runs as step 30 of 34 in the pre-push validation (`pnpm b4push`). The strict image checker follows it as step 31, before HTML validation:
...
The canonical, authoritative b4push step list lives in the header comment of `scripts/run-b4push.sh` — treat that script as the single source of truth for the full 34-step sequence.
