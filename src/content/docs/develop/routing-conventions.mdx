...
The self-contained documentation-route stubs in the scaffold are retained as
explicit host-owned seams, not because injected dynamic routes fail in dev.
zfb 2.13.1 renders those injected routes in both dev and build. The `/` route
remains a separate exception because zfb does not permit injecting `/`.
