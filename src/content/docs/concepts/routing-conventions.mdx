...
If you're migrating from Astro, `paths()` is zfb's replacement for `getStaticPaths()`.
...
In Astro, `getStaticPaths()` was an `async` function that ran in Node.js at build time.
It could `await` anything: file reads, database queries, or live HTTP requests.
