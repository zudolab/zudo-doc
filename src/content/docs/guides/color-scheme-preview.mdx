...
The color scheme preview lets you browse 50+ preset color schemes and apply them to your site in real-time. This functionality lives on the **Color** tab of the [Design Token Panel](../reference/design-token-panel.mdx), via a "Scheme..." dropdown in the tab header.
...
Each scheme specifies a 16-color palette, background/foreground colors, selection colors, optional semantic color overrides, and an optional `shikiTheme` (a vestigial field with no effect on rendering — code highlighting is handled by syntect; see [Syntax Highlighting](../markdown-features/syntax-highlighting.mdx)).
