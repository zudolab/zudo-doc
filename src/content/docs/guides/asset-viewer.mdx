...
`versioned: false` keeps the link pointed at the single default-locale route instead of adding an archived-version prefix. See [Header Navigation](./header-navigation.mdx#properties-versioned) for the full item contract.
...
Individual asset viewers are leaf pages, while the optional route-prefix page is an asset listing. Both exist only in the default locale and stay out of the docs sidebar, search index, `llms.txt`, and sitemap. A reference from any additional-locale document therefore opens the same default-locale asset route as a primary-locale reference.
