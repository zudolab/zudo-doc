...
The five top-level `zudoDoc()` fields are:
...
`assetViewerExclude` applies to both viewer-page generation and the index, while excluded files remain available at their raw public URLs. Metadata sidecars are also omitted from the listing. Like individual viewer pages, the index exists only in the default locale.
...
export default defineConfig(
  zudoDoc({
    ...
    assetViewerIndex: true,
    headerNav: [
      { label: "Files", path: "/files/", versioned: false },
    ],
  }),
);
```

`versioned: false` keeps the link pointed at the single default-locale route instead of adding an archived-version prefix. See [Header Navigation](./header-navigation.mdx#versioned) for the full item contract.
...
Individual asset viewers are leaf pages, while the optional route-prefix page is an asset listing. Both exist only in the default locale and stay out of the docs sidebar, search index, `llms.txt`, and sitemap. A reference from a Japanese document therefore opens the same default-locale asset route as an English reference.
