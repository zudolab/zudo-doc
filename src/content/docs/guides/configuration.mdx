...
Enables the AI assistant chat widget. When `true`, a chat button appears in the page that opens an AI-powered assistant for answering questions about your documentation. The chat API runs as a Cloudflare Pages SSR endpoint (`pages/api/ai-chat.tsx`) — requires `ANTHROPIC_API_KEY`, `DOCS_SITE_URL`, and a `RATE_LIMIT` KV namespace configured in `wrangler.toml`.
...
- **adapter**: Cloudflare Pages — wraps the SSR bundle into `dist/_worker.js`, required for any route exporting `prerender = false`
