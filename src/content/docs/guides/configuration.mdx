...
When enabled, the dev server automatically redirects URLs without a trailing slash to their trailing-slash version with a **301 redirect** (e.g., `/docs/guides` → `/docs/guides/`) so navigation behaves the same way locally as on production. Static assets (files with extensions like `.js`, `.css`, `.png`) and framework-internal paths (`/_astro/`, `/_image` — preserved for cross-framework asset compatibility, as assets historically emitted under those prefixes are still respected) are excluded from the redirect.
