...
You can also replace `public/img/logo.svg` with any custom SVG file. The logo is rendered as a CSS mask, so only the shape matters — the fill color is controlled by the site's color tokens.
