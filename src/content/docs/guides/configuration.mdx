...
stub calls `createChrome(routeCtx)` without host bindings, so the `DocHistory` slot stays at
its no-op default. Thread `chromeBindingsModule` and bind `DocHistory` — see
[Custom Components](./custom-components.mdx) and
...
Setting this config field alone renders nothing in a scaffolded project: the doc-route
stub calls `createChrome(routeCtx)` without host bindings, so the `docHistoryMeta` slot stays at
its `{}` default. Thread `chromeBindingsModule` and bind `docHistoryMeta` to the build-time history
manifest — see [Custom Components](./custom-components.mdx) and
[Host Chrome Bindings](../reference/host-chrome-bindings.mdx). Scaffolds generated
after the [template fix](https://github.com/zudolab/zudo-doc/issues/2716) thread the presentational/data slots automatically;
your own hydrating islands still need the static-import threading described in the guide.
