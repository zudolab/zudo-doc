...
When enabled, the dev server automatically redirects URLs without a trailing slash to their trailing-slash version with a **301 redirect** (e.g., `/docs/guides` → `/docs/guides/`) so navigation behaves the same way locally as on production. Static assets (files with extensions like `.js`, `.css`, `.png`) and the Astro-legacy paths (`/_astro/`, `/_image` — preserved for backward-compat with existing/cached links; zfb does not emit these prefixes) are excluded from the redirect.
...
Color schemes are defined in `src/config/color-schemes.ts`. Each scheme provides 21 color properties:

- `background`, `foreground`, `cursor`, `selectionBg`, `selectionFg`
- `palette` — 16 color slots (`palette[0]` through `palette[15]`)
- `semantic` (optional) — overrides for `surface`, `muted`, `accent`, `accentHover`, and more
