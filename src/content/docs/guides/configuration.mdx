...
Controls whether URLs end with a trailing slash. When `true`, all internal links are generated with a trailing slash (e.g., `/docs/guides/` instead of `/docs/guides`). The build-time URL helpers in `@zudo-doc/zudo-doc-v2/url-normalizer` enforce this policy across every emitted route — see [Trailing-Slash Policy](../concepts/trailing-slash-policy.mdx) for how the rule splits between build time and the deployment host.
...
In dev mode, history is served by the standalone `@zudo-doc/doc-history-server` package running on port 4322. In production, a CLI generator creates static JSON files during CI. See the [Doc History Server reference](../reference/doc-history-server.mdx) for details on the server and CLI.
