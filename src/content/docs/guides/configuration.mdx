...
Href values starting with `/` are prefixed with the configured [`base`](#base), so `"/favicon.svg"` resolves to `/pj/my-site/favicon.svg` on a sub-path deployment. Anything else — an absolute `https://…` URL, a `data:` URL — is emitted verbatim.

<Warning>

[`head.alternateLinks`](#head) cannot replace this set. Those entries render **after** the favicon block, so a `{ rel: "icon" }` entry there only appends one more link — it can never remove or override one of these. `favicon` is the only way to change what the set contains.
