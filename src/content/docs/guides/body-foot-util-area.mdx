...
Top-level `docHistoryUi: false` hides the History button and DocHistory island in this area and disables history JSON generation and the dev proxy. `bodyFootUtilArea.docHistory: true` cannot re-enable it. Git-based metadata and the view-source link are retained. See [dates-only history configuration](./doc-history.mdx).
...
where `contentDir` is the page's content directory (for example, `src/content/docs` for the primary locale, `src/content/docs-ja` for an additional locale, or the `docsDir` of the active version) and `entryId` is the page's file path within that directory.
...
### Additional-locale fallback pages

If an additional-locale page falls back to primary-locale content, the link still points to the actual file that rendered — the primary-locale file in `src/content/docs/` in that case.
