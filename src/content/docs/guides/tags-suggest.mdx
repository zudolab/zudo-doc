...
`pnpm tags:suggest` runs the package-owned suggestion CLI; it is a developer-facing helper, not part of the build. The generated package script passes the same explicit `src/config/tag-vocabulary.ts` `TagCliConfig` used by `tags:audit`. The CLI reads one or more doc files, asks a locally-running LLM to propose up to three tag ids from that vocabulary, and either shows them in an interactive prompt or appends them to a JSON Lines file for batch review.
