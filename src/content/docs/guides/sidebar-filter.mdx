...
The sidebar filter is built into the `SidebarTree` Preact component, shipped by `@takazudo/zudo-doc` as the `sidebar-tree-island` component. It is always rendered as part of the sidebar — there is no separate setting in `settings.ts` to toggle it. To remove it from a scaffolded project, [eject the `sidebar-tree-island` component](../reference/customizing.mdx) and modify the filter logic directly.
