...
The built-in `theme-toggle` component is enough for most sites, but some consumers want their own toggle markup — a different icon, an extra label, custom placement. If you build a custom toggle, **import `ThemeToggle` from the dedicated `@takazudo/zudo-doc/theme-toggle` subpath** and wrap your own UI around it (or re-export it under the same name). Do not hand-copy a local `ThemeToggle` component into your project.
...
The package's `applyColorScheme` (in `theme-toggle/color-scheme-sync.ts`) deliberately does **not** clear the design-token tweak-state keys (`zudo-doc-tweak-state` / `zudo-doc-tweak-state-v2`) when the color scheme changes — zdtp owns its own storage lifecycle. If you use the design-token tweak panel and you are migrating from a hand-rolled toggle that used to wipe tweak state on toggle, be aware the package toggle preserves it instead.
