...
The `design-token-panel` trigger button dispatches the `toggle-design-token-panel` custom event on `window`. The zdtp panel (`@takazudo/zudo-design-token-panel`) listens for this event natively — no additional wiring is needed.
