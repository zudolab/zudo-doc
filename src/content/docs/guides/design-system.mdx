...
zudo-doc uses a **tight token strategy** — all Tailwind defaults are reset and replaced with a small, intentional set of design tokens. This page covers the full system: spacing, typography, colors, border radius, and breakpoints.
...
zudo-doc solves this by resetting defaults with wildcard `initial` resets in `src/styles/global.css`:
...
After these resets, only explicitly defined tokens work. Using a reset token (like `p-4` or `bg-gray-500`) produces no effect — the value simply doesn't exist. This gives you a guarantee: **invalid tokens cause build errors or missing styles, not visual bugs**.
...
See the [Color guide](/docs/guides/color) for full details on the color token system, color schemes, and customization.

## Usage Rules

<Warning title="Tailwind defaults are disabled">
All default Tailwind values are reset to `initial`. Only project-defined tokens work.
</Warning>
...
```html
<!-- DON'T: Tailwind defaults — these are reset and produce nothing -->
