...
This showcase retains a few project-specific integrations: `client-router-bootstrap`, `desktop-sidebar-toggle`, and the `preset-generator` island. The Design Token Panel bootstrap is package-owned and reaches the island scanner through the static package route → chrome → derive import chain. These are showcase implementation details, not a starter inventory for new projects.
