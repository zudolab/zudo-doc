...
Current islands in the project: `toc.tsx`, `mobile-toc.tsx`, `sidebar-toggle.tsx`, `sidebar-tree.tsx`, `theme-toggle.tsx`, `doc-history.tsx`, zdtp panel (Design Token Panel — self-mounted via `configurePanel()` from the `@takazudo/zudo-design-token-panel` package, not registered in the island registry), `ai-chat-modal`, `find-bar`.
