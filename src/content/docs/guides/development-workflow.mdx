...
| `pnpm b4push` | Pre-push validation: 17-step pipeline (format check → ... → build → link check → HTML validation → preview smoke); E2E is parked |
...
Run `pnpm b4push` before pushing to verify the full site builds and all links resolve. It runs 17 steps (format check through preview smoke); E2E tests are intentionally parked and not part of this pipeline.
