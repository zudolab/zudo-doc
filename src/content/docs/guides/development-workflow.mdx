...
| `pnpm b4push` | Pre-push validation: 13-step pipeline (format check → ... → build → link check → HTML validation → preview smoke); E2E is parked |
...
Run `pnpm b4push` before pushing to verify the full site builds and all links resolve. It runs 13 steps (format check through preview smoke); E2E tests are intentionally parked and not part of this pipeline.
...
Current islands in the project: `toc.tsx`, `mobile-toc.tsx`, `sidebar-toggle.tsx`, `sidebar-tree.tsx`, `theme-toggle.tsx`, `doc-history.tsx`, zdtp panel (Design Token Panel — self-mounted via `configurePanel()` from the `@takazudo/zdtp` package, not registered in the island registry), `ai-chat-modal`, `find-bar`.
