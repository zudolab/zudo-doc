...
This page covers the **left-side tabs**. The right-side cluster (appearance menu, GitHub link, etc.) is configured separately — see [Header Right Items](./header-right-items.mdx).
...
| `path` | string | URL path the link navigates to |
| `categoryMatch` | string (optional) | Links this child to a separate top-level sidebar category; omit it for nested paths under the parent's category |
| `versioned` | boolean (optional) | Whether this item's link carries the active `/v/{version}` prefix. Default `true` |
...
A [note tray](./note-tray.mdx) is not a special header-nav type. Add it like any other top-level category: set the item's `path` to the tray index and its single-segment `categoryMatch` to the tray directory name. The tray declaration in `index.mdx` controls its sidebar and listing behavior.
...
#### Nested Children Under One Category

When every child is nested below one top-level category, keep `categoryMatch` only on the parent. Child active state is resolved from each child's `path`, while the parent's single-segment `categoryMatch` keeps the sidebar scoped to the complete top-level category. Per-package changelogs are the canonical example:
...
Do not copy the parent's `categoryMatch` onto these children, and do not use a multi-segment value such as `categoryMatch: "changelog/core"`. `categoryMatch` only matches a top-level directory; a multi-segment child value never matches and can empty the sidebar. The build warns when two or more children share the same `categoryMatch` and whenever a parent or child uses a multi-segment value.
...
If your project enables [versioning](./versioning.mdx), audit every `headerNav` item that links to unversioned content and set `versioned: false` on it. The showcase's own "Claude" and "Codex" header items (from the `claudeResources` and `codexResources` features) are textbook cases: those routes are generated only for the default `docsDir` and never exist inside an archived version directory, so both items set `versioned: false`.
