...
Do not copy the parent's `categoryMatch` onto these children, and do not use a multi-segment value such as `categoryMatch: "changelog/core"`. `categoryMatch` only matches a top-level directory; a multi-segment child value never matches and can empty the sidebar. The build warns about duplicated parent/child values and multi-segment child values.
