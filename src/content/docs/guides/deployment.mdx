...
The preview-only config also adds `global_fetch_strictly_public`, which Cloudflare requires for same-zone Worker fetches; production's compatibility flags remain unchanged.
