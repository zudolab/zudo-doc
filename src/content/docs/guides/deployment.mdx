...
Triggered on pull requests targeting `main`. Runs ten jobs:
...
3. **typecheck** — runs `pnpm check` (TypeScript type checking)
4. **package-tests** — runs all package-scoped vitest suites
5. **root-tests** — runs the root unit test suite
6. **build-site** — full clone (`fetch-depth: 0`), `pnpm build`, then `check:links` (no `SKIP_DOC_HISTORY` — same rationale as production: preBuild needs full history for SSG meta dates)
7. **html-validate** — validates the built `dist/` for HTML element errors
8. **build-history** — full clone, doc history generation
9. **e2e** — runs Playwright end-to-end tests
10. **preview** — merges artifacts and uploads a preview via `wrangler versions upload --preview-alias`, then posts the URL as a PR comment
