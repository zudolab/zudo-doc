...
Its entry keeps the public request intact for the generated adapter but probes `ASSETS` with an internal hostname. Cloudflare's asset binding matches only the pathname; this avoids treating the preview alias as a same-zone Worker fetch. Production continues to export the generated adapter directly.
