...
Each PR gets a **stable preview-alias** URL via `wrangler versions upload --preview-alias <alias>`. The generated adapter-only preview config keeps `preview_urls = true` and enables `workers_dev`; production keeps its primary `workers.dev` route disabled. The alias is `pr-<number>` for PR checks and the branch slug for branch previews. The URL is on `*.workers.dev` (not the production custom domain) and its subdomain starts with the alias:
...
When the isolated service is first created, CI explicitly enables its workers.dev and preview-URL subdomain settings through the Cloudflare API, then retries the aliased upload.
