...
Its entry sends GET/HEAD directly to `ASSETS` with an internal hostname and delegates other methods to the generated adapter. Cloudflare's asset binding matches only the pathname; this avoids treating the preview alias as a same-zone Worker fetch while retaining the API POST smoke gate. Production continues to export the generated adapter directly.
...
migration, production `RATE_LIMIT` KV, or production's custom-domain route; the live AI admission and
rate-limit path is production-only.
...
The JSON decision table below assumes `docHistory: true` and `docHistoryUi: true`. With `docHistoryUi: false`, the plugin skips JSON generation and the dev proxy regardless of environment variables. The preBuild manifest is retained unless `SKIP_DOC_HISTORY=1` blanks it; `DOC_HISTORY_SKIP_POSTBUILD=1` is redundant. Remove a host’s separate `build-history` job and artifact handoff steps too: the standalone generator CLI is not disabled by this setting. See the [Document History guide](./doc-history.mdx) for configuration and the full interaction table.
