...
If you take that opt-out, restrict the whole section: the overview prefix `/docs/claude/` has to be listed alongside `/docs/claude-md/`, `/docs/claude-skills/`, `/docs/claude-agents/`, and `/docs/claude-commands/`. With the overview prefix missing, the locale-owned overview index is still generated into every configured locale, and an authored locale `claude/index.mdx` without `generated: true` fails the build with `resource-docs: refusing to overwrite authored locale index`. See [A resource section includes its overview prefix](./i18n.mdx#default-locale-only-prefixes-a-resource-section-includes-its-overview-prefix).
...
External URLs, root-relative URLs, and page fragments are retained. Link examples inside inline code or fenced code are unchanged. Ordinary authored MDX continues to follow the [documentation linking rules](../getting-started/writing-docs.mdx).
