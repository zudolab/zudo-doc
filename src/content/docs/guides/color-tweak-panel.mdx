...
## Interaction with Color Scheme Picker

The tweak panel and the [Color Scheme Preview](/docs/guides/color-scheme-preview) picker work independently:

- The **scheme picker** switches between pre-defined schemes
- The **tweak panel** modifies individual color values of the currently active scheme

If you switch schemes via the picker, the tweak panel will read the new scheme's values on next open. Tweak state in `localStorage` takes precedence over the active scheme until you click **Reset all**.
