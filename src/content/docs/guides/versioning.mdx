...
Versioning is **disabled by default**. To enable it, add a `versions` array to `zudoDoc({...})` in `zfb.config.ts`:
...
| `locales` | `Record<string, { dir: string }>` | No | Per-locale content directories; each entry takes only `{ dir }` (no `label`) |
...
├── docs-de/             # Latest version — German (if configured)
├── docs-v1/             # Version 1.0 — English
├── docs-v1-ja/          # Version 1.0 — Japanese
├── docs-v1-de/          # Version 1.0 — German (if configured)
├── docs-v0.9/           # Version 0.9 — English
├── docs-v0.9-ja/        # Version 0.9 — Japanese
└── docs-v0.9-de/        # Version 0.9 — German (if configured)
```

<Warning>

Each version directory must exist and contain valid MDX content files. zudo-doc creates a separate zfb content collection for each version, so missing directories will cause build errors.
...
- **Version 1.0 (German):** `/v/1.0/de/docs/getting-started/introduction/`
...
The page is auto-generated from the `versions` array you pass to `zudoDoc({...})`. No manual maintenance is needed — adding or removing versions in the configuration automatically updates the listing page.

Each version's documentation link (and the "latest docs" link) points at the route slug configured by `entryDocSlug`:
...
`entryDocSlug` is a route slug with no leading or trailing slashes — e.g. `"getting-started"` or `"overview/getting-started"`. It defaults to `"getting-started"` when omitted, and is not validated at runtime: pointing it at a slug that doesn't exist simply produces a 404 link rather than failing the build.
...
2. **Copy each configured locale directory** if applicable:

   ```bash
   cp -r src/content/docs-ja src/content/docs-v2-ja
   cp -r src/content/docs-de src/content/docs-v2-de
   ```
