...
The build warns when a configured `sourceDir` points at the landing directory or when a package directory has no non-index release files.
...
The generator creates the landing page and one package index for each slug. The version-bump skill then updates the selected packages: it adds a new version section to each single-page changelog, or a new per-version file to each package using the per-version-file layout.
