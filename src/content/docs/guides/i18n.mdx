...
The default locale is served at `/docs/...` with no language code in the path, while every other locale uses its key as a prefix — `/ja/docs/...` for Japanese. This falls out of `defaultLocale` plus the route split in `pages/` (`pages/docs/[[...slug]].tsx` vs `pages/[locale]/docs/[[...slug]].tsx`); there is no `prefixDefaultLocale` option to set. Changing `defaultLocale` requires updating the corresponding routing and collection wiring in `zfb.config.ts`.
...
The ten prefixes above cover the Claude- and Codex-resource sections. They are the auto-generated pages that surface resources from the project's `.claude/` and `.codex/` directories, plus `AGENTS.md` files and `.agents/skills/` directories. These pages are English-only because their source material is not translated.

<Note>

The top-level `/docs/claude/` and `/docs/codex/` index pages are **not** in this list — they have bilingual stubs at `docs-ja/claude/index.mdx` and `docs-ja/codex/index.mdx`. Only the ten deeper prefixes listed above are default-locale-only.
