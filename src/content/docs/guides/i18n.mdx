...
Each `locales` entry requires `label` and `dir`. In this example, the Japanese hero uses its own description; German uses the global `siteDescription`.
...
1. Add its exact code, display label, and content directory to `locales`. Optionally set `description` to translate its home page hero text:
...
`dateFormat` accepts per-locale overrides, and on a multilingual site that is normally the form you want: `Aug 22, 2026` and `2026年8月22日` are both correct for their locale, and no single pattern serves both.

On every dated surface, the render locale is the page locale, including on single-locale sites; it is no longer derived from whether a locale switcher is present.
...
Each role resolves as `locales[locale][role]` → the top-level `[role]` → `"locale"`. A locale with no entry in `dateFormat.locales` simply uses the top-level roles. See [`dateFormat`](./configuration.mdx#content-and-i18n-dateformat) for the five roles, the token vocabulary, and the rest of the rules.

<Warning title="Month names are localized for three locales only">

Month **names** — the `MMM` and `MMMM` tokens, and the built-in `"locale"` rendering — resolve through a fixed map: `en` → `en-US`, `ja` → `ja-JP`, `de` → `de-DE`. Every other locale code falls back to `en-US`.

So for a locale outside that map, say `fr`:

- A numeric pattern such as `"DD/MM/YYYY"` renders correctly — digits do not depend on the locale.
- A pattern containing `MMM` or `MMMM` still renders **English** month names.
- Leaving a role at `"locale"` renders that role in the `en-US` shape.

Give such a locale a numeric pattern. A month-name pattern will not produce its language.
...
### A resource section includes its overview prefix

The four sub-section prefixes above are not the whole Claude section. The overview page lives under its own prefix, `/docs/claude/` — `/docs/codex/` for Codex — and a prefix list that omits it leaves the overview localized while the sub-sections are restricted.

That matters at build time, not just in navigation. The overview index is locale-owned: with its prefix unlisted, the generator writes it into every configured locale directory. If an authored `src/content/docs-<locale>/claude/index.mdx` is already sitting at that target and its frontmatter lacks `generated: true`, the build stops with:

```text
resource-docs: refusing to overwrite authored locale index
```

Pick one of the two shapes and apply it to the whole section:

- **Default-locale-only** — list the overview prefix alongside the sub-section prefixes, so nothing under `/docs/claude/` is generated for other locales.
- **Localized** — leave the prefixes unlisted, and remove or rename the authored locale index so the generator can own it. Move its title, description, and labels to `ZudoDocConfig.translations`.
