...
Bind the real History island in the bindings module:
...
Point the same config at that module:
...
When creating a project, the generator patches the document-route stubs for this binding only when the doc-history feature was selected. Add the module and route-stub threading manually to an existing scaffold; use the static-import form in [Custom Components](./custom-components.mdx) for this hydrating island.
...
In dev mode (`pnpm dev`), the doc-history-server runs as a standalone REST API server on port 4322, launched concurrently with the zfb dev server via `run-p`. The doc-history zfb plugin's `devMiddleware` hook proxies `/doc-history/*` requests from the browser to this server.
