...
Each page's body is indexed up to the [`searchMaxBodyLength`](./configuration.mdx#search-searchmaxbodylength) setting — 3,000 characters by default — so a query can match text deep into a long page, not only its opening lines. This setting controls indexing depth only, not what a result shows: a match always renders as a compact snippet of about 200 characters centered on the match, and the page description is shown instead of the body whenever one is set. Raising the cap grows `search-index.json` accordingly, since the whole indexed excerpt of every page ships to the browser — raising it from the old 300-character default to 3,000 grew this showcase's index about 3.4× (roughly 410 KB to 1.4 MB).

<Note title="The Search Worker shares the same body text">

The optional [Search Worker](#server-side-search-search-worker) indexes the same `searchMaxBodyLength`-capped body text, so changing the setting affects both the built-in widget and the worker together.
...
Use the `SearchWidget` slot in [Host Chrome Bindings](../reference/host-chrome-bindings.mdx) to replace the header search widget. The chrome passes exactly these seven locale-aware props:
...
For setup instructions, API documentation, and deployment details, see the [Search Worker API reference](../reference/search-worker.mdx).
