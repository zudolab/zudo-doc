...
3. Symlinks the primary `src/content/docs/` directory into the skill folder
4. Symlinks every configured additional-locale directory (for example, `src/content/docs-ja/` and `src/content/docs-de/`) into the skill folder
5. Symlinks the skill into `~/.claude/skills/` and/or `~/.codex/skills/` so it's available globally
...
├── docs -> src/content/docs/         # Symlink to primary-locale docs
├── docs-ja -> src/content/docs-ja/   # Configured locale link (if present)
└── docs-de -> src/content/docs-de/   # Another configured locale link (if present)
```

The link names follow the configured directory basenames (or a code-qualified
fallback when a custom directory collides). The generated `SKILL.md` records
the exact locale map; version snapshots such as `docs-v1-ja/` are not linked as
current locales. Keep translations structurally aligned and remember that
non-JA starter trees contain English placeholder prose until you translate
them.
