...
The feature is **enabled by default**. To explicitly control it, set `llmsTxt` in `zfb.config.ts`:
...
This supplements the standard path-based discovery (`/llms.txt` at the domain root) which may not work when your site uses a `base` path like `/my-docs/`.

## Dev Mode

The generated files are available during development via zfb's devMiddleware. When running `pnpm dev`, requests to `/llms.txt` and `/llms-full.txt` are handled dynamically — no build step needed.
