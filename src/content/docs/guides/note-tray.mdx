...
Quote `date` and `updated` values so YAML keeps them as strings.
...
| `dated note-tray children require date` | Add a quoted, calendar-valid `date` to every item, including `unlisted` items |
