...
See the [Frontmatter reference](../frontmatter.mdx#fields-hide_toc) for full documentation.

If you want readers to make that call themselves rather than fixing it per page, enable the [`tocToggle`](../configuration.mdx#site-settings-toctoggle) setting — it puts a persistent show/hide control on the pages that render the default desktop table of contents. This page is not one of them: `hide_toc: true` drops the TOC at build time, so there is nothing left to toggle.

## See Also

- [Hide Sidebar](./hide-sidebar.mdx) — hides the left sidebar
