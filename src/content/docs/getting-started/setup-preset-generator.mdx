...
To customize the palette beyond these two starting points, edit the ramps and semantic map in `src/config/color-schemes.ts` after scaffolding — see [Color](../reference/color.mdx#adding-a-custom-color-scheme) and preview changes live with the [Design Token Panel](../reference/design-token-panel.mdx)'s Palette tab (see [Color Scheme Preview](../guides/color-scheme-preview.mdx)).
