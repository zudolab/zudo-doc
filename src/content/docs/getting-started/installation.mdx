...
The selected language is the primary locale: its content lives in `src/content/docs/` and its routes are unprefixed (`/docs/...`). To add more locales, enter an ordered comma-separated list in the **Additional language codes** field of the Preset Generator, or pass the list directly to the CLI:
...
An omitted or blank list leaves the scaffold single-language. A non-empty explicit list is normalized, validated for safe locale path segments, and implies i18n; it is the complete list rather than a request for one fixed locale. The scaffold seeds English placeholder prose for arbitrary non-Japanese locales, which you can translate after installation.
...
zudo-doc ships exactly these two schemes, sharing one set of OKLCH ramps — there is no catalog of community or terminal presets to browse. The defaults are package-owned; a fresh scaffold has no color-scheme module. To customize the palette, create your own scheme module and pass it through the `colorSchemes` field of `zudoDoc({ ... })` in `zfb.config.ts` (see the [Color guide](../reference/color.mdx#adding-a-custom-color-scheme)), then preview changes live with the [Design Token Panel](../reference/design-token-panel.mdx)'s Palette tab (see [Color Scheme Preview](../guides/color-scheme-preview.mdx)).

### Features

Use the interactive picker to choose optional modules for search and sidebar tools, Claude and Codex resources and skills, design and navigation controls, versioning and document history, tags and changelogs, desktop wrappers, and indexing. Internationalization is configured by the primary language plus the **Additional language codes** field; a non-empty additional list enables it automatically. Built-in full-text/word-match search, sidebar filtering, image enlarge, dynamic page transitions, and footer copyright are selected by default.

For the complete feature list, defaults, and non-interactive flags, see the [CLI reference](../reference/create-zudo-doc.mdx#options-features).
...
When additional locales are configured, the same `src/content/` tree also contains one exact `docs-<code>/` directory per entry, such as `docs-ja/` and `docs-de/`. Keep filenames and code blocks aligned across these trees while translating prose. See [Internationalization](../guides/i18n.mdx) for fallback, validation, and default-locale-only rules.
...
Package route injection renders your doc pages in both **build** and **dev** with zfb 2.13.1. The small self-contained stub remains as an explicit host-owned route seam: it reconstructs the doc route from the package's sanctioned entrypoints (`@takazudo/zudo-doc/route-context` + `/chrome` + the `virtual:zudo-doc-route-context` module) and lets the generated project customize that route while it shadows the package injection. It makes `/docs/…` return 200 in both `zfb dev` and `zfb build`.
