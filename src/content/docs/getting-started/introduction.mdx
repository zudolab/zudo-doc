...
- **Color scheme support** — themeable ramp-based palette (base + accent + state colors) with light and dark defaults
- **Static export** — generates pure HTML, zero JS by default (Preact islands only where needed)
- **i18n ready** — configure any number of locale routes and content trees
...
The color scheme system uses a ramp-native palette (shared base/accent/state ramps resolved to semantic roles). By swapping a single color scheme, the entire site's appearance changes.
...
Then open http://localhost:4321 to see your docs. See the [Installation](./installation.mdx) guide for all CLI options and setup details.
