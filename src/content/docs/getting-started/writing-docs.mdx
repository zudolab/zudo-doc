...
Components that need interactivity register with zfb's island runtime through their own `useEffect` / event-binding code. Components that only render static HTML render server-side and ship zero JavaScript.
