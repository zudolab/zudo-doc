...
The default scaffold has one locale and no `src/content/docs-ja/` directory or locale route. Select the `i18n` feature during scaffolding to add a secondary language, its mirrored content directory, and `pages/[locale]/docs/[[...slug]].tsx`. You can also add locales later through the `locales` field in `zudoDoc({ ... })`.
...
A plain component import renders server-side only and ships zero JavaScript. To register a component for use across documents or to add an interactive island, follow [Custom Components](../guides/custom-components.mdx): it uses `mdxExtras` for global registration and the experimental `Island()` recipe for client hydration.
