...
- The `admonitionsPreset` variants (`note`, `tip`, `info`, `warning`, `danger`, `details`) are provided by the opt-in [Admonitions Preset](./admonitions-preset.mdx) feature, not by the Core registry itself. The registry is the mechanism; the preset supplies the component mappings.
