---
title: Code Enrichment
description: Adds a copy button and diff/line/word-highlight decorations to fenced code blocks.
sidebar_position: 17
---

The `codeEnrichment` feature decorates fenced code blocks with a copy button and per-line/per-word annotations — diff markers, line highlighting, and word highlighting. It runs as a hast-phase visitor after syntax highlighting, so it operates on the already-tokenized `<span class="line">` output.

:::note[Opt-in feature]

This feature is disabled by default. Enable it with the `codeEnrichment` key in `zfb.config.ts`. It is an object-typed feature, so it must be given an options object (`{}` or fields) — the `true` shorthand is rejected.

:::

:::note[Title bar is a separate, always-on feature]

The filename/title bar shown above a titled fence (`.code-block-container` / `.code-block-title`) comes from a separate, always-on core mechanism — not from `codeEnrichment`. It renders whether or not `codeEnrichment` is enabled. See [Code Title](./code-title.mdx).

:::

## Configuration

```ts title="zfb.config.ts"
export default defineConfig({
  markdown: {
    features: {
      codeEnrichment: {},
    },
  },
});
```

All three enhancements (diff markers, line highlighting, and word highlighting) are active by default. Disable any individually:

```ts title="zfb.config.ts"
codeEnrichment: {
  diffMarkers: false,
  lineHighlight: true,
  wordHighlight: true,
}
```

## Diff markers

Append `// [!code ++]` or `// [!code --]` comments at line ends. The marker is stripped from the output, and the line receives a `data-line-diff` attribute (`added` or `removed`):

```js title="diff-demo.js"
const unchanged = 1;
const removed = 2; // [!code --]
const added = 3; // [!code ++]
```

Supported comment styles match the language: `//` (JavaScript/TypeScript/Rust), `#` (Python/Ruby/Shell), `--` (SQL/Lua).

## Line highlighting

Add brace-delimited ranges to the fence info-string. Matching lines receive a `data-line-highlight="true"` attribute:

```js title="highlight-demo.js"
const a = 1;
const b = 2;
const c = 3;
const d = 4;
const e = 5;
```

The fence above was authored as:

````md
```js {1,3-5}
const a = 1;
const b = 2;
const c = 3;
const d = 4;
const e = 5;
```
````

Supported range syntax: single numbers (`{3}`), inclusive ranges (`{3-5}`), and combinations (`{1,3-5,8}`).

## Word highlighting

Add whitespace-delimited `/phrase/` expressions to the fence info-string. Every visible occurrence of the phrase in the block is wrapped in a `.highlighted-word` span, preserving syntax-highlighting markup at the match boundaries:

```js title="word-demo.js" /answer/
const answer = 42;
console.log(answer);
```

The fence above was authored as:

````md
```js title="word-demo.js" /answer/
const answer = 42;
console.log(answer);
```
````

Multiple expressions are space-separated (`/first/ /second phrase/`); a literal slash inside a phrase is escaped as `\/` (`/path\/name/`). Malformed or unterminated expressions are silently ignored.

## Copy button

The copy button is added by client-side JavaScript at runtime; it is not present in the server-rendered HTML. When hydrated, the button markup is `button.code-btn.code-btn-copy` containing `svg.code-icon.code-icon-copy`.

## Emitted DOM

| Trigger | Emitted element(s) | Class names |
| --- | --- | --- |
| Diff marker line | `<span class="line" data-line-diff="added\|removed">` | `span.line[data-line-diff]` |
| Highlighted line | `<span class="line" data-line-highlight="true">` | `span.line[data-line-highlight]` |
| Word match | wraps the matched visible text | `.highlighted-word` |
