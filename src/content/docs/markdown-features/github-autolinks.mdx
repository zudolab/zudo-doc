...
`zudoDoc()` wires this feature through the `githubAutolinksRepo` setting. Set it to your `"owner/repo"` string and the preset enables `githubAutolinks` for you — omit it and the feature stays inactive. A bare `defineConfig({ markdown: { features } })` bypasses this setting and discards the rest of the preset:
