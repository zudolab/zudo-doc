...
Enable it with the `imageEnlarge` key in `zfb.config.ts`:
...
- The wrap is idempotent — a paragraph already containing a `figure.zd-enlargeable` is not processed again.
