...
The **GitHub Alerts** feature converts GitHub-style alert blockquotes — the `> [!NOTE]` syntax you see in README files — into the same admonition markup produced by the [Admonitions Preset](./admonitions-preset.mdx). This lets you author callouts with the familiar GitHub syntax and have them rendered identically to `:::note`-style directives.
...
The output is identical to the [Admonitions Preset](./admonitions-preset.mdx) — for example `> [!NOTE]` produces:
...
- GitHub Alerts and the [Admonitions Preset](./admonitions-preset.mdx) coexist: GitHub alerts are processed before the admonitions pass, so you can enable both at once.
