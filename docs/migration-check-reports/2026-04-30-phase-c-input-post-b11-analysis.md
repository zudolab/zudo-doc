...
- New wrapper files in `pages/lib/`:
  - `_category-nav.tsx` — wrap `CategoryNav` from `@takazudo/zudo-doc/nav-indexing`. Reads `category` prop, calls `buildNavTree()` + `findNode()` host-side, passes resolved children as data props.
  - `_category-tree-nav.tsx` — same shape for `CategoryTreeNav`.
  - `_site-tree-nav-demo.tsx` — same shape for `SiteTreeNavDemo`.
  - `_details.tsx` — wrap `Details` from `@takazudo/zudo-doc/details`. Likely no host-side data needed; just forward children + props.
