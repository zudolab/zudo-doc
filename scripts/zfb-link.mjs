...
function main() {
  ...
  // `ZFB_ESBUILD_BIN` pre-populated (when the project ships its own
  // esbuild). Without this every developer has to discover and export
  // the variable manually before `pnpm exec zfb build` works.
  ...
  try {
    ...
    console.log(`[zfb:link] ${linkPath} → wrapper for ${target}` + (esbuildBin ? `\n[zfb:link]   ZFB_ESBUILD_BIN → ${esbuildBin}` : ""));
  } catch (err) {
