...
// itself exit non-zero on a fallback — but it has NO allowlist, so it can
// only be turned on where every shipped route is expected to be clean.
// `pnpm b4push` deliberately builds with `--no-strict-content-bridge`
// instead, so this post-hoc dist/ scan (which DOES have an allowlist) is
// the thing that gates it — needed because b4push's local dist/ can contain
// gitignored local-only routes (`docs/claude-skills/` and `docs/codex-skills/`,
// generated by the Claude and Codex watchers / `pnpm setup:doc-skill`, never
// present in a CI checkout).
//
// The `docs/claude-skills/` and `docs/codex-skills/` allowlist entries are
...
// tied to a specific character class, and `escapeForMdx()` only defends
// against one trigger class. Do NOT retire this script just because zfb now
// fails the build itself — the two guards never run on the same build and
// neither covers the other's case: the native gate is the only one CI runs,
// and this scan is the only one b4push can run; see
