...
#   7. Current-only compatibility contract
#   8. Design token lint
...
#  15. B4push/CI parity check (guard manifest meta-check — #1967)
#  16. Type checking (zfb check + workspace package typechecks)
#  17. Worker contract proof (types + Workers runtime + Wrangler dry-run)
#  18. Root unit tests (test:unit) — fast src/scripts specs; builds @takazudo/zudo-doc as a side-effect
#  19. Slow root unit tests (test:unit:slow) — subprocess-heavy root specs
#  20. Package tests (test:packages) — ~1,535 suite tests across workspace packages (as of 2026-07)
#  21. Package safelist check (#1994) — requires dist/safelist.css from step 18
#  22. Build (zfb build)
#  23. Content-fallback check (#3134) — no page may ship a <pre data-zfb-content-fallback> body
#  24. Link check
#  25. HTML validation (html-validate dist/**/*.html)
#  26. Automated preview smoke (blocking)
#  27. Manual interactive smoke (operator-driven)
#
...
# its gen-z-index/check-z-index codegen) was byte-identical to the package
# default (@takazudo/zudo-doc/theme.css ships the same 13 tiers
# unconditionally), so it was dead weight once the showcase stopped
# customizing it.
#
...
#   B4PUSH_SKIP_HTML_VALIDATE=1  — skip HTML validation (step 24)
#   B4PUSH_SKIP_PREVIEW_SMOKE=1  — skip the automated preview smoke (step 25)
...
# (e.g. step 16 "Type checking") without needing every pass/fail/skip call
...
# Steps 1–15 are lightweight guard gates. They are delimited by the markers
...
# ── Step 7: Current-only compatibility contract ─────
step "Current-only compatibility contract"
if (cd "$ROOT_DIR" && pnpm check:compatibility-contract); then
  pass "Current-only compatibility contract passed"
else
  fail "Current-only compatibility contract"
fi

# ── Step 8: Design token lint ────────────────────────
...
# ── Step 13: Search-widget-script commit drift check (#3421, #3431) ──
...
# gitignored routes-src/ convention, see its own header comment), so this is
# a real drift guard rather than a stale-artifact check. See
# scripts/check-search-widget-drift.sh.
step "Search-widget-script commit drift check (check:search-widget-drift)"
if (cd "$ROOT_DIR" && pnpm check:search-widget-drift); then
  pass "Search-widget-script commit drift check passed"
else
...
if (cd "$ROOT_DIR" && pnpm --filter @takazudo/zudo-doc check:prepack-contract); then
  pass "@takazudo/zudo-doc publish contract passed"
else
  fail "@takazudo/zudo-doc publish contract"
fi

# ── Step 15: B4push/CI parity check ──────────────────
...
# dist/safelist.css ready for the safelist check in step 20.
#
...
# dist/ is already built by step 18 — no extra prep needed.
...
# ── Step 21: Package safelist check ──────────────────
...
# Requires dist/safelist.css — produced by the package build in step 18.
...
# gate (#3234) so this build still produces a dist/ for step 23's
...
# ── Step 23: Content-fallback check ───────────────────
#
...
# ── Step 26: Automated preview smoke (blocking) ──────
...
# ── Step 27: Manual interactive smoke ────────────────
