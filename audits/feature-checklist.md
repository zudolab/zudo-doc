...
- **Settings key**: current key(s) in `@takazudo/zudo-doc/config` (the showcase supplies concrete values from `src/config/settings.ts`)
...
| configuration | guides/configuration.mdx | Current `ZudoDocConfig` fields | `@takazudo/zudo-doc/config` + showcase `src/config/settings.ts` values | no | Central reference for current settings, defaults, and effects | yes | S9 |
...
| doc-history | guides/doc-history.mdx | `docHistory`, `bodyFootUtilArea` | `@takazudo/zudo-doc/plugins/doc-history`, island `DocHistory`, `@takazudo/zudo-doc-history-server` | yes | Shows git commit history per doc page; dev mode proxies to the standalone server on port 4322; CI generates JSON files in parallel | yes | S12 |
| doc-skill-symlinker | guides/doc-skill-symlinker.mdx | `claudeResources` | `@takazudo/zudo-doc/plugins/claude-resources` | no | Generates MDX pages from `.claude/` skills/commands/agents; pre-build step writes content into docs collection | yes | S13 |
...
| llms-txt | guides/llms-txt.mdx | `llmsTxt` | `@takazudo/zudo-doc/plugins/llms-txt` | no | Emits `llms.txt` and `llms-full.txt` (plus per-locale variants) at build time; dev middleware serves them live | yes | S13 |
| search | guides/search.mdx | `headerRightItems` (search component) | `@takazudo/zudo-doc/plugins/search-index`, `packages/search-worker/` | partial | Full-text search index emitted at build; `SearchWidget` renders a dialog with results; Cloudflare Worker serves server-side search; keyboard shortcut `/` or Ctrl+K opens | yes | S10 |
...
| tag-governance | guides/tag-governance.mdx | `tagGovernance`, `tagVocabulary` | package docs schema, `src/utils/tags.ts`, package-owned bins | no | Governance modes: `off`/`warn`/`strict`; exact vocabulary IDs are canonical and strict mode rejects unknown tags | yes | S8 |
| tags-audit | guides/tags-audit.mdx | `tagGovernance`, `tagVocabulary` | `@takazudo/zudo-doc` `tags-audit` bin (`pnpm tags:audit`) | no | CLI audit reports unknown tags; `--ci` fails for hard findings | yes | S8 |
| tags-suggest | guides/tags-suggest.mdx | `tagVocabulary` | package-owned `tags-suggest` bin (`pnpm tags:suggest`) | no | CLI suggests canonical tag ids for pages with missing or unrecognised tags | yes | S8 |
| tags | guides/tags.mdx | `docTags`, `tagPlacement`, `tagVocabulary` | package-owned tag area and injected tag routes | no | Tags displayed on each doc page per `tagPlacement`; tag index pages list pages per exact canonical ID | yes | S8 |
...
| design-token-panel | reference/design-token-panel.mdx | `designTokenPanel` | package-owned `DesignTokenPanelBootstrap`, `@takazudo/zdtp` | yes | Tabbed panel for live token tweaking; persistence and import/export are dependency-owned | yes | S9 |
...
The authoritative field census is the `ZudoDocConfig` interface and
`DEFAULT_SETTINGS` in `packages/zudo-doc/src/config.ts`; this audit does not
duplicate a frozen field list.
