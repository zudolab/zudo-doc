...
| `tags` (cross-category grouping) | PASS | — | — | `dist/docs/guides/tags/index.html` contains tag badges; tag index at `dist/docs/tags/` has per-tag pages. `resolvePageTags()` in `src/utils/tags.ts` validates exact canonical IDs and deduplicates them. | None |
...
| `tagGovernance: "off"` (vocabulary not consulted) | PASS (code-trace) | — | — | `resolveTag()` passes raw tags through with `known: false`; `buildTagsSchema()` returns a loose `z.array(z.string())`. | None |
| `tagVocabulary: true` (vocabulary consulted) | PASS | — | — | Exact vocabulary IDs are recognized; all other values are unknown. Grouped footer rendering remains available. | None |
...
| `pnpm tags:audit --ci` (CI mode) | PASS | — | — | The package-owned `tags-audit` bin reads the explicit `--config src/config/tag-vocabulary.ts` module and reports unknown canonical IDs. | None |
| `tags-suggest` (local LLM suggester) | PASS (doc + package bin verified) | — | — | `package.json` invokes the package-owned `tags-suggest --config src/config/tag-vocabulary.ts` bin. | None |
...
Orphan vocabulary IDs are a soft audit finding. Remove an unused entry or add authored canonical usage; there is no deprecation field in the current schema.
