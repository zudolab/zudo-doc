...
| basic-components — typography overrides (h2–h4, p, a, strong, blockquote, ul/ol, table) | FAIL | regress | zfb-upstream | `dist/docs/components/basic-components/index.html` is in `<pre data-zfb-content-fallback>` mode; no `htmlOverrides` bindings reached; `src/components/content/` Preact files exist but MDX compilation aborted | Resolve fallback root cause (see below); once fixed, verify `<h2 class="text-title …">` etc. rendered by `heading-h2.tsx` etc. |
