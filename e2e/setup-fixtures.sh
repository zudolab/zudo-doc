...
EOF
  ...
  # ----- wrangler.toml — fixture-local Workers static-assets config -----
  # When a fixture build emits dist/_worker.js, `zfb preview` runs in "adapter
  # mode" and hands off to `wrangler dev` (Workers mode — zfb >= next.74; the
  # bump e77b323f switched preview off `wrangler pages dev`, matching zfb's
  # Workers-assets output: dist/_worker.js + .assetsignore, and this repo's
  # Pages->Workers migration #1691). `wrangler dev` searches upward for the
  # nearest wrangler.toml; without a fixture-local one it would climb to the
  # repo-root Workers config, which carries a custom-domain [[routes]] entry
  # and KV/preview bindings that don't belong to a throwaway fixture preview.
  # So we emit a minimal fixture-local Workers static-assets config mirroring
  # the repo root's shape (main + [assets]) but stripped of routes/KV/preview,
  # shadowing the root so each fixture previews self-contained.
  #
  # A Pages-style config (`pages_build_output_dir = "dist"`) is NOT usable here:
  # `wrangler dev` rejects Pages projects with "It looks like you've run a
  # Workers-specific command in a Pages project" and preview aborts.
  #
  ...
  # socket that defaults to a fixed port (9229+); booting all five previews at
  # once (playwright webServer) makes multiple workerd instances race to bind
  # the same inspector port and the losers die with "Address already in use
  # (127.0.0.1:92xx)" (zudolab/zudo-doc#2084). `wrangler pages dev` was more
  # forgiving here; `wrangler dev` is not, so the timing stagger in
  # playwright.config.ts is no longer reliable on its own. Pinning to 0 removes
  # the race at the root — each instance gets its own free port. (0 = random
  # free port; the stagger stays as belt-and-suspenders.)
  cat > "$fixture_dir/wrangler.toml" <<EOF
# Generated by e2e/setup-fixtures.sh — fixture-local Workers static-assets
# config so \`zfb preview\` (adapter mode -> wrangler dev, zfb >= next.74)
# resolves THIS fixture instead of climbing to the repo-root wrangler.toml
# (which carries a custom-domain route + KV/preview bindings).
name = "zudo-doc-e2e-${fixture}-fixture"
